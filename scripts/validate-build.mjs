...
const js=['assets/js/common-v3.68-r65.js','assets/js/avatars-v3.68-r65.js','assets/js/city-v3.68-r65.js','assets/js/admin-v3.68-r65.js','assets/js/display-v3.68-r65.js','assets/js/player-v3.68-r65.js','assets/js/simulator-v3.68-r65.js','assets/js/motion-v3.68-r65.js','assets/js/pro-admin-v3.68-r65.js','assets/js/pro-runtime-v3.68-r65.js'];
const css=['assets/css/core-v3.68-r65.css','assets/css/admin-v3.68-r65.css','assets/css/display-v3.68-r65.css','assets/css/display-ui-v3.92-r65.css','assets/css/player-ui-v3.92-r65.css','assets/css/simulator-v3.68-r65.css','assets/css/avatars-runtime-v3.68-r65.css','assets/css/avatars-ui-v3.92-r65.css','assets/css/motion-v3.68-r65.css','assets/css/pro-v3.68-r65.css'];
...
// 2
let ids=0;const supa='https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2.112.4/dist/umd/supabase.min.js';for(const rel of html){const s=read(rel),arr=[...s.matchAll(/\bid=["']([^"']+)["']/gi)].map(x=>x[1]),seen=new Set();ids+=arr.length;for(const id of arr){if(seen.has(id))fail(`${rel}: id duplicado ${id}`);seen.add(id);}for(const m of s.matchAll(/\b(?:src|href)=["']([^"']+)["']/gi)){checkRef(rel,m[1]);if(/^assets\/(?:js|css|vendor)\//.test(m[1])&&!m[1].includes('v=3.68-r65'))fail(`${rel}: cache-bust r65 ausente -> ${m[1]}`);}if(rel!=='simulator.html'&&!s.includes(supa))fail(`${rel}: Supabase JS pin 2.112.4 ausente`);if(!s.includes('3.68-r65'))fail(`${rel}: build r65 ausente`);}ok(`${html.length} HTML / ${ids} IDs / refs principais verificados.`);
// 3
for(const r of [...js,...css])if(!exists(r))fail(`asset r65 ausente: ${r}`);ok('Conjunto ativo r65 existe.');
// 4
...
// 6
req('assets/js/common-v3.68-r65.js',["BUILD_ID='3.68-r65'","BACKEND_SCHEMA_REQUIRED=42","TEAM_FEATURE_SCHEMA_REQUIRED=43"]);req('VERSION.txt',['RELEASE=r65','BUILD_ID=3.68-r65','BACKEND_SCHEMA_REQUIRED=42','BACKEND_SCHEMA_RECOMMENDED=43','TEAM_FEATURE_SCHEMA_REQUIRED=43','SUPABASE_MIGRATIONS=001-043']);ok('Handshake r65: base schema 042, equipes avançadas schema 043.');
// 7
...
// 14
req('assets/js/admin-v3.68-r65.js',['renderBackendFeatureState','BACKEND_SCHEMA_REQUIRED','TEAM_FEATURE_SCHEMA_REQUIRED','schema>=BACKEND_SCHEMA_REQUIRED','schema>=TEAM_FEATURE_SCHEMA_REQUIRED','00_ATUALIZAR_SUPABASE_PARA_SCHEMA_043.sql']);req('assets/js/pro-admin-v3.68-r65.js',['proBackendSchema','setTeamSchemaAvailability','schema 043','Os demais modos funcionam no schema 042']);ok('Schema 042 não bloqueia aplicação; recurso de equipes é feature-gated no 043.');
// 15
req('assets/js/player-v3.68-r65.js',['BACKEND_SCHEMA_REQUIRED','checkPlayerBackend']);req('assets/js/display-v3.68-r65.js',['BACKEND_SCHEMA_REQUIRED','checkDisplayBackend']);ok('Jogador e telão usam o requisito base schema 042.');
// 16
req('assets/js/admin-v3.68-r65.js',['resetNewRoomQueueIfNeeded','admin_remove_queue_item','reuseQuestionsForNextRoom','newGameReuseQuestions','Nova sala criada com a seleção de perguntas vazia']);req('admin.html',['newGameReuseQuestions','Uma nova sala sempre começa vazia']);ok('Nova sala vazia por padrão + reutilização explícita de perguntas.');
// 17
req('assets/js/admin-v3.68-r65.js',['continueCurrentRoom','reviewCurrentRoom','presentationContextForRoom','centralStepGo','renderCentralJourney']);ok('Central respeita contexto Padrão/GameShow.');
// 18
req('assets/js/admin-v3.68-r65.js',['templateTeamSetup','restoreTemplateTeams','template_team_setup','admin_save_teams','admin_set_team_join_mode']);ok('Modelos/duplicação preservam configuração das equipes.');
// 19
req('admin.html',['id="proQuickCreateCard"','id="proGameModeCard"','data-pro-pane="room"','Nome + dinâmica antes de criar']);forbid('admin.html',['data-pro-pane="dynamics"'],'subaba dinâmica separada');req('assets/js/pro-admin-v3.68-r65.js',["name==='dynamics'?'room'",'createProRoom']);ok('GameShow cria Sala + dinâmica no mesmo passo.');
// 20
req('assets/js/pro-admin-v3.68-r65.js',['updateProStepLabels',"proDraftMode==='teams'",'wizard-step-hidden']);ok('Numeração GameShow é dinâmica com/sem Equipes.');
// 21
req('admin.html',['data-standard-advanced="1"','standardShowAdvancedBtn']);req('assets/css/admin-v3.68-r65.css',['.ui-simple-mode [data-standard-advanced="1"]','standard-show-advanced']);ok('Partida Padrão oculta opções avançadas no modo simples.');
// 22
req('assets/js/admin-v3.68-r65.js',['gameQuestionIssue','validateGameQuestions',"type==='precision'",'correct_order','correct_matches','winning_lines','segments','boxes']);req('assets/js/pro-admin-v3.68-r65.js',['Mecânicas especiais completas','validateGameQuestions']);ok('Checklist valida mecânicas especiais antes do lobby.');
// 23
...
// 24
req('assets/js/admin-v3.68-r65.js',['launchGuidedTest','admin_list_teams','voteTypes','Math.max(players,8)','openLoadSimulator(Math.min(100,players)']);ok('Teste guiado adapta quantidade de jogadores virtuais.');
// 25
req('admin.html',['id="configQuestionBankMount"','id="proQuestionBankMount"','id="sharedQuestionBankWorkspace"']);req('assets/js/admin-v3.68-r65.js',['mountQuestionBank','questionBankContext']);ok('Banco de perguntas contextual Padrão/GameShow preservado.');
// 26
...
// 27
const proRun=read('assets/js/pro-runtime-v3.68-r65.js');if(proRun.includes('MutationObserver'))fail('pro-runtime contém MutationObserver');req('assets/js/common-v3.68-r65.js',['rankSignature','el.dataset.rankSignature===signature']);req('assets/js/motion-v3.68-r65.js',['semanticClassName','restartHandles','requestAnimationFrame',"filter(c=>!c.startsWith('motion-'))"]);forbid('assets/js/motion-v3.68-r65.js',['void el.offsetWidth','let last=el.className']);ok('Proteções anti-loop/anti-flicker preservadas.');
// 28
if(read('admin.html').includes('motion-v3.68-r65.js')||read('index.html').includes('motion-v3.68-r65.js'))fail('motion pré-carregado antes de login/join');req('assets/js/admin-v3.68-r65.js',['loadAdminMotionAfterAuth']);req('assets/js/player-v3.68-r65.js',['loadPlayerMotionAfterJoin']);ok('Motion continua deferido após autenticação/entrada.');
// 29
const av=read('assets/js/avatars-v3.68-r65.js'),keys=[...av.matchAll(/\{key:'([^']+)'/g)].map(x=>x[1]);if(new Set(keys).size!==31)fail(`avatares ${new Set(keys).size}/31`);for(const k of new Set(keys))for(const d of ['runtime-r30','preview-r30'])if(!exists(`assets/avatars/${d}/${k}.webp`))fail(`avatar ausente ${d}/${k}`);const city=read('assets/js/city-v3.68-r65.js');for(const sc of ['office','laboratory','industry','platform'])if(!city.includes(`${sc}:{`))fail(`mapa ausente ${sc}`);ok('31 avatares + quatro mapas preservados.');
// 30
const jsTop=walk('assets/js').filter(x=>x.endsWith('.js')),cssTop=walk('assets/css').filter(x=>x.endsWith('.css'));if(jsTop.length!==11)fail(`assets/js deveria ter 11 arquivos ativos/config; tem ${jsTop.length}`);if(cssTop.length!==10)fail(`assets/css deveria ter 10 arquivos ativos; tem ${cssTop.length}`);for(const f of jsTop.filter(x=>x!=='assets/js/config.js'))if(!f.includes('r65'))fail(`JS legado no runtime: ${f}`);for(const f of cssTop)if(!f.includes('r65'))fail(`CSS legado no runtime: ${f}`);if(exists('assets/avatars/runtime-r21')||exists('assets/avatars/hd'))fail('diretórios legados de avatar ainda presentes');ok('Runtime enxugado: assets JS/CSS e avatares legados removidos.');
// 31
for(const r of ['README.md','CHANGELOG_v3.68_r65_CI_Consistente.txt','VALIDATION_r65_CI_Consistente.txt','VERSION.txt','01_Abrir_Simulador_QuizRounds2_v3.68-r65.bat','00_VERIFICAR_ANTES_DO_PUSH.bat','.github/workflows/pages.yml','admin/index.html'])if(!exists(r))fail(`release file ausente ${r}`);req('.github/workflows/pages.yml',['Deploy QuizRounds2 no GitHub Pages','Ler versão da release','node scripts/validate-build.mjs','node scripts/build-pages.mjs','node scripts/validate-pages.mjs _site','BUILD_ID: ${{ steps.release.outputs.build }}']);forbid('.github/workflows/pages.yml',['Deploy QuizRounds2 r65','Validar release r65','Montar site estático r65']);req('scripts/build-pages.mjs',['parseVersionFile','version.json diverge de VERSION.txt','sb_publishable_','SUPABASE_URL','SUPABASE_PUBLISHABLE_KEY']);req('admin/index.html',['../admin.html?v=3.68-r65']);const cfg=read('assets/js/config.js');if(!cfg.includes('COLE_AQUI_A_URL_DO_PROJETO')||!cfg.includes('COLE_AQUI_APENAS_A_CHAVE_SB_PUBLISHABLE'))fail('config.js não neutro');ok('Release docs/CI auto-versionado/alias/configuração neutra verificados.');
// 32
req('assets/css/avatars-ui-v3.92-r65.css',["industry-v3.68-r65.webp",'--city-ratio:1.333333','r65 — mapas refinados + escala de avatar compatível','--city-avatar-size:47px']);if(exists('assets/maps/industry-v3.50.webp'))fail('mapa antigo da Indústria ainda ativo no pacote');if(!exists('assets/maps/industry-v3.68-r65.webp'))fail('mapa refinado da Indústria ausente');ok('Mapa da Indústria r65 + proporção nativa 4:3 + escala compacta verificados.');
// 33
...
req('assets/js/city-v3.68-r65.js',['spawns somente no grafo transitável principal',"['w3','crew5']","city.querySelector('.city-walkers')"]);ok('Spawns/POIs dos quatro mapas pertencem ao mesmo grafo transitável e o mascote fica contido no mapa.');
// 34
req('assets/js/avatars-v3.68-r65.js',["et_skunk',label:'Gambá ET'","gamba:'et_skunk'","gambazinho:'et_skunk'","special_skunk:'et_skunk'"]);req('assets/css/avatars-ui-v3.92-r65.css',['.city-walkers{z-index:2000}','position:relative;z-index:4']);req('assets/css/motion-v3.68-r65.css',['z-index:2060']);ok('Avatar especial e prioridade visual de nomes/personagens protegidos.');
// 35
req('admin.html',['data-avatar-scene-live="office"','data-avatar-scene-live="laboratory"','data-avatar-scene-live="industry"','data-avatar-scene-live="platform"']);req('assets/js/admin-v3.68-r65.js',['setAvatarSceneQuick','avatar_scene:next','overlayReset.error','Não foi possível limpar a sobreposição do telão']);req('assets/js/display-v3.68-r65.js',['applyAvatarScene','settings?.avatar_scene']);ok('Troca de mapa ao vivo e abertura do telão sem overlay residual verificadas.');
// 36
for(const rel of js){const z=read(rel);if(/sb_secret_[A-Za-z0-9_-]+/.test(z)||/service_role\s*[:=]\s*["'][^"']+/i.test(z))fail(`${rel}: segredo privilegiado encontrado`);}req('assets/js/admin-v3.68-r65.js',['ensureControllerForRoomCreation','settingsDraftDirty','admin_close_and_score_round_r47']);req('assets/js/pro-runtime-v3.68-r65.js',['pro_player_team_state','pro_player_choose_team','submit_quiz_game_answer']);ok('Hardening de credenciais, controlador, regras e runtime Pro preservado.');
// 37
req('assets/js/common-v3.68-r65.js',['isDisplayAudioScope','publishDisplayAudioState','quizrounds:display-audio-state','playFeedbackCue','isFeedbackAudioReady','setDisplayAudioVolume','startDisplayAmbience','stopDisplayAmbience','allReady','countdown','questionOpen','lastFive','timeEnd','newLeader','finalRanking','freeze']);
req('assets/js/display-v3.68-r65.js',["playCue('allReady'","playCue('countdown'","playCue('questionOpen'","playCue('lastFive'","playCue('timeEnd'","playCue('newLeader'","playCue('finalRanking'","playCue('freeze'",'displayAudioGate','activateDisplayAudio',"const revealed=phase==='result'",'pro_ambient_audio','startDisplayAmbience','stopDisplayAmbience']);
forbid('assets/js/display-v3.68-r65.js',['AudioContext','createOscillator']);
forbid('assets/js/player-v3.68-r65.js',['playFeedbackCue','unlockFeedbackAudio','setFeedbackAudioEnabled','AudioContext','createOscillator','playCue(','soundToggle']);
forbid('assets/js/admin-v3.68-r65.js',['playFeedbackCue','unlockFeedbackAudio','setFeedbackAudioEnabled','AudioContext','createOscillator','adminCue(']);
forbid('assets/js/pro-admin-v3.68-r65.js',['AudioContext','createOscillator','playFeedbackCue']);
forbid('assets/js/pro-runtime-v3.68-r65.js',['AudioContext','createOscillator','playFeedbackCue','setFeedbackAudioEnabled','playProCue','ambientSchedule','ambientCtx','userInteracted','lastPhaseCue']);
const activeRuntimeJs=walk('assets/js').filter(x=>x.endsWith('.js')&&x!=='assets/js/config.js'),webAudioOwners=activeRuntimeJs.filter(rel=>/\b(?:AudioContext|webkitAudioContext)\b/.test(read(rel)));if(webAudioOwners.length!==1||webAudioOwners[0]!=='assets/js/common-v3.68-r65.js')fail(`WebAudio deve existir somente em common-v3.68-r65.js; encontrado em: ${webAudioOwners.join(', ')||'nenhum'}`);
forbid('index.html',['id="soundToggle"']);req('display.html',['id="displayAudioGate"','Ativar som do telão']);
req('assets/css/display-ui-v3.92-r65.css',['.display-main-area>.display-panel-enter','city-scene-crossfade-old','display-audio-gate']);
ok('AudioManager único no telão, master mute e cues sem duplicação Pro verificados.');
// 38
req('assets/js/pro-runtime-v3.68-r65.js',["isPlayer&&(type==='audio'||type==='video')",'pro-media-remote','Mídia no telão','Acompanhe o áudio e o vídeo pela apresentação principal.','syncDisplayMediaAudio','displaySoundEnabled','quizrounds:display-audio-state']);req('assets/css/pro-v3.68-r65.css',['.pro-media-remote']);forbid('assets/js/player-v3.68-r65.js',['<audio','<video','new Audio(']);ok('Mídia sonora/audiovisual no celular é substituída por aviso para acompanhar no telão.');
// 39
req('version.json',['"release": "r65"','"build": "3.68-r65"','"backend_schema_required": 42']);
...
forbid('.github/workflows/pages.yml',["printf '%s' \"$body\" | grep -Fq","Deploy QuizRounds2 r65 no GitHub Pages"]);req('.github/workflows/pages.yml',["[[ \"$body\" == *\"$needle\"* ]]"]);ok('Pipeline GitHub Pages auto-versionado gera, valida e aguarda propagação sem falso negativo por SIGPIPE.');
// 40
...
const jsonMeta=JSON.parse(read('version.json'));if(versionMeta.RELEASE!=='r65'||versionMeta.BUILD_ID!=='3.68-r65'||jsonMeta.release!==versionMeta.RELEASE||jsonMeta.build!==versionMeta.BUILD_ID)fail('fonte de verdade da release divergente entre VERSION.txt e version.json');
for(const rel of ['.github/workflows/pages.yml','scripts/build-pages.mjs','scripts/validate-pages.mjs'])if(/r6[0-4]/.test(read(rel)))fail(`${rel}: referência de release anterior encontrada no CI atual`);ok('Fonte de verdade da release e CI sem mistura de versões anteriores.');
// 41
