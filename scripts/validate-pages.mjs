...
for(const rel of ['index.html','admin.html','display.html','simulator.html','admin/index.html','version.json','VERSION.txt','assets/js/config.js','assets/js/common-v3.68-r62.js'])req(rel);
for(const rel of ['index.html','admin.html','display.html','simulator.html','admin/index.html']){
  const s=read(rel);if(!s.includes('3.68-r62'))errors.push(`${rel}: marcador 3.68-r62 ausente`);
}
const marker=JSON.parse(read('version.json'));
if(marker.build!=='3.68-r62'||marker.release!=='r62')errors.push('version.json não identifica r62/3.68-r62');
...
const common=read('assets/js/common-v3.68-r62.js');
if(!common.includes("BUILD_ID='3.68-r62'"))errors.push('common.js publicado não é r62');
...
console.log('VALIDAÇÃO _site: APROVADA — r62 pronta para upload-pages-artifact.');
