import{db,$,esc,renderRank,setConnection,serverRemaining,sleep,APP_VERSION,BUILD_ID,BACKEND_SCHEMA_REQUIRED,TEAM_FEATURE_SCHEMA_REQUIRED,applyDocumentTheme,normalizeLogoUrl}from'./common-v3.68-r91.js';
import{avatarMarkup}from'./avatars-v3.68-r91.js';
...
function loadAdminMotionAfterAuth(){
  ...
  if(!document.getElementById('adminMotionCss')){const link=document.createElement('link');link.id='adminMotionCss';link.rel='stylesheet';link.href='assets/css/motion-v3.68-r91.css?v=3.68-r91';document.head.appendChild(link);}
  import('./motion-v3.68-r91.js?v=3.68-r91').catch(()=>{});
}
...
function renderBackendFeatureState(){
  ...
  if(schema>=TEAM_FEATURE_SCHEMA_REQUIRED){banner.classList.add('ready');if(title)title.textContent='Backend completo';if(text)text.textContent='Todos os recursos, inclusive os 3 modos de formação de equipes, estão disponíveis.';if(badge){badge.textContent=`SCHEMA ${String(schema).padStart(3,'0')}`;badge.className='badge ready';}}
...
async function checkBackendCompatibility({announce=false}={}){
  try{
    ...
    backendMeta=data||null;backendCompatible=!error&&schema>=BACKEND_SCHEMA_REQUIRED&&sameRelease&&buildOk;renderBackendFeatureState();
  ...
  }catch(error){backendCompatible=false;backendMeta=null;renderBackendFeatureState();diag('error','Não foi possível verificar a versão do backend',{error:error.message||String(error)});if(announce)alert(`Não foi possível validar o backend. O schema mínimo é ${String(BACKEND_SCHEMA_REQUIRED).padStart(3,'0')}.`);return{ok:false,error};}
}
...
async function createExampleGame(){
  if(exampleGamePromise){actionToast('A Partida de exemplo já está sendo criada.');return exampleGamePromise;}
  const task=(async()=>{
    const previousRoomId=String(room?.id||'');
    const btn=$('#createExampleGameBtn');
    const title=$('#roomTitle'),rounds=$('#plannedRounds');
    const previousTitle=title?.value??'',previousRounds=rounds?.value??'';
    ...
    await createRoom();
    if(!room?.id||(previousRoomId&&String(room.id)===previousRoomId)){
      if(title)title.value=previousTitle;
      if(rounds)rounds.value=previousRounds;
      return false;
    }
    const ids=await ensureExampleQuestionIds();
    for(const id of ids){
      const queued=await db.rpc('admin_queue_question',{p_room_id:room.id,p_question_id:id});
      if(queued.error)throw queued.error;
    }
    const planned=await db.rpc('admin_update_planned_rounds',{p_room_id:room.id,p_planned_rounds:2});
    if(planned.error)throw planned.error;
    room.planned_rounds=2;
  ...
  try{return await task;}
    ...
    diag('error','Falha ao criar Partida de exemplo',{error:e?.message||String(e)});
    actionToast(e?.message||'Falha ao criar Partida de exemplo','error');
    alert(e?.message||String(e));
    return false;
  }finally{
    if(exampleGamePromise===task)exampleGamePromise=null;
    const btn=$('#createExampleGameBtn');if(btn)btn.disabled=false;
  }
}
