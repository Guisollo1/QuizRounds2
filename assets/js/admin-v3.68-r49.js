import{db,$,esc,renderRank,setConnection,serverRemaining,sleep,APP_VERSION,BUILD_ID,BACKEND_SCHEMA_REQUIRED,applyDocumentTheme,normalizeLogoUrl}from'./common-v3.68-r49.js';
import{avatarMarkup}from'./avatars-v3.68-r49.js';
...
function loadAdminMotionAfterAuth(){
  ...
  if(!document.getElementById('adminMotionCss')){const link=document.createElement('link');link.id='adminMotionCss';link.rel='stylesheet';link.href='assets/css/motion-v3.68-r49.css?v=3.68-r49';document.head.appendChild(link);}
  import('./motion-v3.68-r49.js?v=3.68-r49').catch(()=>{});
}
...
function initAdminTabs(){if($('#adminTabs')?.dataset.ready)return;$('#adminTabs').dataset.ready='1';$$('.admin-tab').forEach(btn=>btn.addEventListener('click',()=>switchAdminTab(btn.dataset.tab)));let saved='central';try{saved=localStorage.getItem('quiz2AdminTab')||'central';}catch{}switchAdminTab(['central','questions','config','pro','presentation'].includes(saved)?saved:'central');}
...
window.quiz2AdminBridge={...(window.quiz2AdminBridge||{}),createGameShowRoom:createGameShowRoomFromPro,getRoom:()=>room,switchTab:(name)=>switchAdminTab(name),setConfigPane:(name)=>setConfigPane(name)};
