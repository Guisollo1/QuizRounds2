import{db,$,esc,renderRank,setConnection,serverRemaining,sleep,APP_VERSION,BUILD_ID,BACKEND_SCHEMA_REQUIRED,applyDocumentTheme,normalizeLogoUrl}from'./common-v3.68-r52.js';
import{avatarMarkup}from'./avatars-v3.68-r52.js';
...
let presentationContext='config',questionBankContext='config';
...
function loadAdminMotionAfterAuth(){
  ...
  if(!document.getElementById('adminMotionCss')){const link=document.createElement('link');link.id='adminMotionCss';link.rel='stylesheet';link.href='assets/css/motion-v3.68-r52.css?v=3.68-r52';document.head.appendChild(link);}
  import('./motion-v3.68-r52.js?v=3.68-r52').catch(()=>{});
}
...
function mountQuestionBank(context='config'){
  ...
  if($('#questionBankBackBtn'))$('#questionBankBackBtn').textContent='← Voltar para Sala';
  if($('#questionBankNextBtn'))$('#questionBankNextBtn').textContent='Continuar: Regras →';
...
function setConfigPane(name,{scroll=false}={}){const panel=$('#adminPanelConfig');if(!panel)return;const pane=['room','questions','rules','rounds','presentation'].includes(name)?name:'room';panel.dataset.configPane=pane;[...panel.querySelectorAll('.config-subtabs [data-config-pane]')].forEach(btn=>{if(!btn.matches('button'))return;const on=btn.dataset.configPane===pane;btn.classList.toggle('active',on);btn.setAttribute('aria-selected',String(on));});try{localStorage.setItem('quiz2ConfigPane',pane);}catch{}if(pane==='questions')mountQuestionBank('config');if(pane==='presentation')mountPresentation('config');if(scroll)panel.querySelector('.config-subtabs')?.scrollIntoView({behavior:'smooth',block:'nearest'});}
...
async function createGameShowRoomFromPro(detail={}){
  ...
  if(room){const risk=roomRiskSummary();if(risk.hasWork){const carried=risk.queued>0?` As ${risk.queued} pergunta(s) selecionada(s) serão copiadas para a nova sala.`:'';if(!confirm(`A sala atual ${room.code} será encerrada ao criar o novo GameShow.${carried}\n\nDeseja continuar?`))return null;}}
  ...
  const ok=await withAction('Criando GameShow…',async()=>{
    ...
    const carried=Number(data?.carried_queue_count||0);room=data;created=data;rememberRoom();roomState=null;queueRows=[];presenceOnline=null;displayOnline=false;lastAdminQr='';eventInsights=null;lastInsightsRounds=-1;settingsDraftDirty=false;settingsDraftSerial=0;
    ...
    if(teams.length>=2){const teamResult=await db.rpc('admin_save_teams',{p_room_id:room.id,p_teams:teams});if(teamResult.error)throw teamResult.error;}
    await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants(),loadRecentRooms()]);await subscribe();
    if(carried>0&&$('#queueWarning')){$('#queueWarning').textContent=`${carried} pergunta(s) mantida(s) da sala anterior. A nova partida está pronta para usar essa seleção.`;$('#queueWarning').className='queue-warning ready';}
...
window.quiz2AdminBridge={...(window.quiz2AdminBridge||{}),createGameShowRoom:createGameShowRoomFromPro,getRoom:()=>room,switchTab:(name)=>switchAdminTab(name),setConfigPane:(name)=>setConfigPane(name),mountPresentation:(context)=>mountPresentation(context),mountQuestionBank:(context)=>mountQuestionBank(context),openPresentation:(context)=>openPresentationContext(context),getPresentationContext:()=>presentationContext};
...
function journeyGo(step){if(step==='room'){switchAdminTab('config');setConfigPane('room',{scroll:true});}else if(step==='questions'){switchAdminTab('config');setConfigPane('questions',{scroll:true});}else if(step==='rules'){switchAdminTab('config');setConfigPane('rules',{scroll:true});}else if(step==='rounds'){switchAdminTab('config');setConfigPane('rounds',{scroll:true});}else if(step==='review'){switchAdminTab('central');setTimeout(()=>$('#setupReviewCard')?.scrollIntoView({behavior:'smooth',block:'start'}),50);renderSetupReview();}else if(step==='lobby'){openPresentationContext(presentationContextForRoom(),{scroll:false});setTimeout(()=>$('#adminLobbyCode')?.scrollIntoView({behavior:'smooth',block:'center'}),50);}}
