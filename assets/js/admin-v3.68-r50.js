import{db,$,esc,renderRank,setConnection,serverRemaining,sleep,APP_VERSION,BUILD_ID,BACKEND_SCHEMA_REQUIRED,applyDocumentTheme,normalizeLogoUrl}from'./common-v3.68-r50.js';
import{avatarMarkup}from'./avatars-v3.68-r50.js';
...
let presentationContext='config';
...
function loadAdminMotionAfterAuth(){
  ...
  if(!document.getElementById('adminMotionCss')){const link=document.createElement('link');link.id='adminMotionCss';link.rel='stylesheet';link.href='assets/css/motion-v3.68-r50.css?v=3.68-r50';document.head.appendChild(link);}
  import('./motion-v3.68-r50.js?v=3.68-r50').catch(()=>{});
}
...
function setConfigPane(name,{scroll=false}={}){const panel=$('#adminPanelConfig');if(!panel)return;const pane=['room','rules','rounds','presentation'].includes(name)?name:'room';panel.dataset.configPane=pane;[...panel.querySelectorAll('.config-subtabs [data-config-pane]')].forEach(btn=>{if(!btn.matches('button'))return;const on=btn.dataset.configPane===pane;btn.classList.toggle('active',on);btn.setAttribute('aria-selected',String(on));});try{localStorage.setItem('quiz2ConfigPane',pane);}catch{}if(pane==='presentation')mountPresentation('config');if(scroll)panel.querySelector('.config-subtabs')?.scrollIntoView({behavior:'smooth',block:'nearest'});}
...
window.quiz2AdminBridge={...(window.quiz2AdminBridge||{}),createGameShowRoom:createGameShowRoomFromPro,getRoom:()=>room,switchTab:(name)=>switchAdminTab(name),setConfigPane:(name)=>setConfigPane(name),mountPresentation:(context)=>mountPresentation(context),openPresentation:(context)=>openPresentationContext(context),getPresentationContext:()=>presentationContext};
