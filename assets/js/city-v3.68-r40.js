import{avatarMapMarkup,applyAvatarFrameElement,normalizeAvatarKey}from'./avatars-v3.68-r40.js';
...
export function createAvatarCity(host,badge,{maxActors=100}={}){
  ...
  function destroy(){destroyed=true;if(timer)clearInterval(timer);timer=null;for(const actor of actors.values())actor.el.remove();actors.clear();if(fxLayer)fxLayer.innerHTML='';if(occlusionLayer)occlusionLayer.innerHTML='';mascotEl?.remove();mascotEl=null;}
