import{db,$,esc,renderRank,setConnection,serverRemaining,sleep,APP_VERSION,BUILD_ID,BACKEND_SCHEMA_REQUIRED,TEAM_FEATURE_SCHEMA_REQUIRED,applyDocumentTheme,normalizeLogoUrl}from'./common-v3.68-r80.js';
import{avatarMarkup}from'./avatars-v3.68-r80.js';
...
function loadAdminMotionAfterAuth(){
  ...
  if(!document.getElementById('adminMotionCss')){const link=document.createElement('link');link.id='adminMotionCss';link.rel='stylesheet';link.href='assets/css/motion-v3.68-r80.css?v=3.68-r80';document.head.appendChild(link);}
  import('./motion-v3.68-r80.js?v=3.68-r80').catch(()=>{});
}
...
async function stabilizeBeforeStart(){
  ...
  const task=(async()=>{
    ...
    if(btn){btn.disabled=true;btn.textContent='Verificando sincronização…';}
    try{
      if(!lastConnectionKey.startsWith('connected')){
        if(eventModeActive){
          const result={ok:false,fallback:false,reason:'Modo Evento exige Realtime conectado'};
          diag('warn','Início bloqueado: Realtime obrigatório no Modo Evento');
          return result;
        }
        const backend=await checkBackendCompatibility(),rpcOk=await refreshState(true);
        if(backend.ok&&rpcOk&&!versionMismatch){
          const result={ok:true,fallback:true,reason:'Realtime indisponível; Supabase/RPC está funcional e o modo normal pode operar temporariamente por polling'};
          diag('warn','Janela de estabilidade aprovada em fallback controlado',{state_version:Number(roomState?.room?.state_version||0),polling:true});
          return result;
        }
        const result={ok:false,fallback:false,reason:backend.ok?'Supabase não respondeu ao estado atual':'backend não pôde ser validado'};
        diag('warn','Início bloqueado durante fallback',{reason:result.reason});
        return result;
      }
      const result=await recoverEventSync({silent:true,timeoutMs:4800});
      if(!result.ok)diag('warn','Início bloqueado por sincronização exata',{reason:result.reason,lagging:result.snapshot?.lagging||0,initializing:result.snapshot?.initializing||0});
      else if(result.ackFallback)diag('ok','Janela de estabilidade aprovada por presença Realtime exata; ACK parcial não bloqueou o início',{state_version:Number(roomState?.room?.state_version||0),responders:result.test?.responders||0,expected:result.test?.expected||0});
      else diag('ok','Janela de estabilidade exata aprovada antes do início',{state_version:Number(roomState?.room?.state_version||0),responders:result.test?.responders||0});
      return result;
    }finally{
