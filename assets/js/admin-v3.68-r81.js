import{db,$,esc,renderRank,setConnection,serverRemaining,sleep,APP_VERSION,BUILD_ID,BACKEND_SCHEMA_REQUIRED,TEAM_FEATURE_SCHEMA_REQUIRED,applyDocumentTheme,normalizeLogoUrl}from'./common-v3.68-r81.js';
import{avatarMarkup}from'./avatars-v3.68-r81.js';

let room=null,roomState=null,queueRows=[],questionBank=[],participants=[],historyRows=[],channel=null,progressChannel=null,poll=null,serverOffset=0,timerTick=null,reconnectTimer=null,reconnectAttempt=0,auditTick=null,actionBusy=false,presenceOnline=null,displayOnline=false,progressRefreshTimer=null,lastAdminQr='',activeAdminTab='central',autoOpening=false,autoClosing=false,lastLatency=0,subscriptionEpoch=0,lastConnectionKey='',refreshSeq=0,refreshAppliedSeq=0,displayWindowRef=null,remoteAuthorized=false,remoteModeActive=false,remoteDeviceLabel='',remotePairLink='',remotePairExpiresAt=0,remoteRoomSyncTimer=null,remoteRoomSyncInFlight=false,eventModeActive=false,controllerGranted=true,controllerConflictLabel='',controllerHeartbeatTimer=null,lastPreflight=null,presenceVersions={players:[],displays:[],admins:[]},presenceDevices={players:[],displays:[],admins:[]},lastPresenceLagCount=-1,versionMismatch=false,eventInsights=null,lastInsightsRounds=-1,lastObservedPhase='',diagnosticEvents=[],lastRemotePhase='',lastRemoteFinalStage='',remoteDeviceCount=0,lastVersionBroadcast='',backendCompatible=false,backendMeta=null,backendCheckStatus='unknown',realtimeTargetVersion=0,realtimeSyncPromise=null,realtimeAuxTimer=null,realtimeFallbackUntil=0,lastAutoRecoveryAt=0,autoRecoveryInFlight=false,startStabilityBusy=false,startStabilityPromise=null,startQuizPromise=null,eventWatchdogTimer=null,lastWatchdogIssue='',lastAutoPreflightAt=0,preflightBusy=false,lastBackendCheckAt=0;
let parsedImport={questions:[],errors:[],results:[]},parseTimer=null,recentQuestionIds=new Set(),connectionTestNonce='',connectionAcks=new Map(),connectionTestFinish=null,connectionTestTimer=null,connectionExpectedTargets=[],connectionTestPromise=null;
...
let presentationContext='config',questionBankContext='config',uiSimpleMode=true,reuseQuestionsForNextRoom=false;
...
function loadAdminMotionAfterAuth(){
  ...
  if(!document.getElementById('adminMotionCss')){const link=document.createElement('link');link.id='adminMotionCss';link.rel='stylesheet';link.href='assets/css/motion-v3.68-r81.css?v=3.68-r81';document.head.appendChild(link);}
  import('./motion-v3.68-r81.js?v=3.68-r81').catch(()=>{});
}
...
function applySimpleMode(simple=true){
  uiSimpleMode=!!simple;document.body.classList.toggle('ui-simple-mode',uiSimpleMode);document.body.classList.toggle('ui-advanced-mode',!uiSimpleMode);
  const btn=$('#simpleModeToggle');if(btn){btn.setAttribute('aria-pressed',String(uiSimpleMode));btn.innerHTML=uiSimpleMode?'<span aria-hidden="true">◎</span> Modo simples':'<span aria-hidden="true">⚙</span> Modo avançado';}
  const proBtn=$('#proSimpleModeBtn');if(proBtn){proBtn.setAttribute('aria-pressed',String(uiSimpleMode));proBtn.textContent=uiSimpleMode?'Modo simples':'Opções avançadas';}
  const hint=$('#proSimpleModeHint');if(hint)hint.textContent=uiSimpleMode?'Recursos avançados ocultos.':'Todos os recursos profissionais visíveis.';
  if(uiSimpleMode&&typeof setRulesMode==='function')setRulesMode('simple');try{localStorage.setItem('quiz2SimpleMode',uiSimpleMode?'1':'0');}catch{}
}
function initSimpleMode(){let simple=true;try{simple=localStorage.getItem('quiz2SimpleMode')!=='0';}catch{}applySimpleMode(simple);}
function openNewGameChooser(){
  const modal=$('#newGameModal');if(!modal)return;
  const wrap=$('#newGameReuseWrap'),check=$('#newGameReuseQuestions');
  if(wrap)wrap.classList.toggle('hidden',!(room&&queueRows.length));
  if(check)check.checked=false;
  modal.classList.remove('hidden');modal.setAttribute('aria-hidden','false');requestAnimationFrame(()=>modal.querySelector('[data-new-game="config"]')?.focus());
}
function closeNewGameChooser(){const modal=$('#newGameModal');if(!modal)return;modal.classList.add('hidden');modal.setAttribute('aria-hidden','true');$('#centralNewGameBtn')?.focus();}
function chooseNewGame(kind){reuseQuestionsForNextRoom=!!$('#newGameReuseQuestions')?.checked;closeNewGameChooser();if(kind==='pro'){switchAdminTab('pro');window.quiz2ProBridge?.setPane?.('room',{remember:true,scroll:true});setTimeout(()=>$('#proRoomTitle')?.focus(),100);}else{switchAdminTab('config');setConfigPane('room',{scroll:true});setTimeout(()=>$('#roomTitle')?.focus(),100);}}
function modeSummaryLabel(){const mode=String(roomSettings()?.game_mode||'').trim();const labels={classic:'GameShow clássico',teams:'GameShow equipes',speed:'GameShow relâmpago',knockout:'GameShow mata-mata',championship:'GameShow campeonato',practice:'GameShow ensaio'};return labels[mode]||'Partida padrão';}
function applyRecommendedRules(){
  if($('#defaultTime'))$('#defaultTime').value=30;if($('#defaultPoints'))$('#defaultPoints').value=100;if($('#shuffleAnswersToggle'))$('#shuffleAnswersToggle').checked=false;
  if($('#breakEvery'))$('#breakEvery').value=0;if($('#autoCloseAll'))$('#autoCloseAll').checked=false;if($('#autoCloseTime'))$('#autoCloseTime').checked=false;
  if($('#speedBonusEnabled'))$('#speedBonusEnabled').checked=false;if($('#streakEnabled'))$('#streakEnabled').checked=false;
  settingsDraftDirty=true;settingsDraftSerial++;updateSettingsSaveIndicator('dirty');renderBuildSummary();renderStandardReview();actionToast('Configuração recomendada aplicada ✓');
}
function renderStandardReview(){
  const list=$('#standardReviewList');if(!list)return;const checks=setupChecks(),done=checks.filter(x=>x.ok).length,steps=['room','questions','rounds','rounds','rules'];
  list.innerHTML=checks.map((x,i)=>`<div class="wizard-review-row ${x.ok?'ok':'warn'}"><span>${x.ok?'✓':'!'}</span><div><strong>${esc(x.label)}</strong><small>${esc(x.detail)}</small></div>${x.ok?'':`<button class="ghost small" data-review-fix="${steps[i]}">Corrigir</button>`}</div>`).join('');
  if($('#standardReviewBadge')){$('#standardReviewBadge').textContent=`${done}/${checks.length}`;$('#standardReviewBadge').className=`badge ${done===checks.length?'ready':'neutral'}`;}
  if($('#standardWizardSummary'))$('#standardWizardSummary').textContent=done===checks.length?`${room?.title||'Partida'} • ${queueRows.length} perguntas • pronta para o lobby`:`Faltam ${checks.length-done} ajuste(s). Clique em Corrigir para ir direto ao item.`;
  if($('#standardOpenPresentationBtn'))$('#standardOpenPresentationBtn').disabled=done!==checks.length;
  list.querySelectorAll('[data-review-fix]').forEach(b=>b.addEventListener('click',()=>journeyGo(b.dataset.reviewFix)));
}
async function launchGuidedTest(){
  if(!room)return actionToast('Crie a sala antes de testar.','error');
  if(!queueRows.length)return actionToast('Escolha pelo menos uma pergunta antes de testar.','error');
  let players=2;const mode=String(roomSettings()?.game_mode||'').trim();
  const voteTypes=new Set(['crowd_prediction','live_poll','audience_choice']);
  if(queueRows.some(q=>voteTypes.has(String(q.game_type||q.game_type_snapshot||'standard'))))players=Math.max(players,8);
  if(mode==='teams'){
    if(backendSchema()<TEAM_FEATURE_SCHEMA_REQUIRED)return actionToast('Para testar equipes, atualize o Supabase para o schema 043.','error');
    try{const{data,error}=await db.rpc('admin_list_teams',{p_room_id:room.id});if(!error)players=Math.max(players,Math.max(2,(data||[]).length)*2);}catch{}
  }
  openLoadSimulator(Math.min(100,players),{full:true});
}
...
function renderDiagnostics(){
  ...
  if($('#diagSync')){const lag=presenceLagSummary();$('#diagSync').textContent=!lastConnectionKey.startsWith('connected')?'Realtime indisponível':versionMismatch?'Versões diferentes':lag.lagging?`${lag.lagging} atrasado${lag.lagging===1?'':'s'} • pior +${lag.worst}`:'Sincronizado';}
...
function presentationContextForRoom(){const mode=String(roomSettings()?.game_mode||'').trim();return mode?'pro':'config';}
function mountPresentation(context='config'){
  const ctx=context==='pro'?'pro':'config',workspace=$('#sharedPresentationWorkspace'),target=$(ctx==='pro'?'#proPresentationMount':'#configPresentationMount');
  if(!workspace||!target)return false;
  presentationContext=ctx;try{localStorage.setItem('quiz2PresentationContext',ctx);}catch{}
  if(workspace.parentElement!==target)target.appendChild(workspace);
  workspace.classList.remove('hidden');workspace.dataset.presentationContext=ctx;
  const pro=ctx==='pro';
  if($('#presentationContextEyebrow'))$('#presentationContextEyebrow').textContent=pro?'CENTRAL AO VIVO · GAMESHOW PRO':'CENTRAL AO VIVO · CONFIGURAÇÃO';
  if($('#presentationContextTitle'))$('#presentationContextTitle').textContent=pro?'Apresentação — GameShow Pro':'Apresentação — Configuração';
  if($('#presentationContextHint'))$('#presentationContextHint').textContent=pro?'Lobby, rounds, equipes e dinâmica da sala GameShow Pro.':'Lobby e rounds da partida criada na Configuração.';
  if($('#presentationContextBadge')){$('#presentationContextBadge').textContent=pro?'GAMESHOW PRO':'CONFIGURAÇÃO';$('#presentationContextBadge').className=`badge presentation-context-badge ${pro?'pro':'neutral'}`;}
  if($('#backToPreparationBtn'))$('#backToPreparationBtn').textContent='← Voltar aos rounds';
  renderAdminQr();if(room){refreshState(false);loadParticipants();}
  return true;
}
function mountQuestionBank(context='config'){
  const ctx=context==='pro'?'pro':'config',workspace=$('#sharedQuestionBankWorkspace'),target=$(ctx==='pro'?'#proQuestionBankMount':'#configQuestionBankMount');
  if(!workspace||!target)return false;
  const changed=questionBankContext!==ctx;questionBankContext=ctx;try{localStorage.setItem('quiz2QuestionBankContext',ctx);}catch{}
  if(workspace.parentElement!==target)target.appendChild(workspace);workspace.dataset.bankContext=ctx;
  if(changed){selectedQuestionIds.clear();updateSelectionToolbar();}
  const pro=ctx==='pro',label=pro?'GameShow Pro':'Partida Padrão';
  if($('#questionBankContextKicker'))$('#questionBankContextKicker').textContent=pro?'GAMESHOW PRO · PASSO 2':'PARTIDA PADRÃO · PASSO 2';
  if($('#questionBankContextTitle'))$('#questionBankContextTitle').textContent=pro?'Escolher perguntas do GameShow':'Escolher perguntas da Partida Padrão';
  if($('#questionBankContextHint'))$('#questionBankContextHint').textContent=pro?'Pesquise, selecione e monte o banco desta sala. Depois você pode enriquecer as perguntas com mídia e mecânicas Pro logo abaixo.':'Pesquise, selecione e monte somente as perguntas que farão parte desta partida.';
  if($('#questionBankContextBadge'))$('#questionBankContextBadge').textContent=pro?'GAMESHOW PRO':'PADRÃO';
  if($('#questionBankContextRoom'))$('#questionBankContextRoom').textContent=room?`Sala ${room.code} · ${room.title||'Quiz'}`:'Nenhuma sala criada';
  if($('#questionBankContextMode'))$('#questionBankContextMode').textContent=label;
  if($('#queueSelectedQuestionsBtn'))$('#queueSelectedQuestionsBtn').textContent=pro?'＋ Adicionar ao GameShow Pro':'＋ Adicionar à Partida Padrão';
  if($('#playlistContextKicker'))$('#playlistContextKicker').textContent=pro?'FILA DO GAMESHOW PRO':'FILA DA PARTIDA PADRÃO';
  if($('#questionBankBackBtn'))$('#questionBankBackBtn').textContent=pro?'← Voltar para Dinâmica':'← Voltar para Sala';
  if($('#questionBankNextBtn')){const nextPro=String(roomSettings()?.game_mode||window.quiz2ProBridge?.getMode?.()||'classic')==='teams'?'Equipes':'Regras';$('#questionBankNextBtn').textContent=pro?`Continuar: ${nextPro} →`:'Continuar: Regras →';}
  renderQuestionBank();return true;
}
function openPresentationContext(context=presentationContextForRoom(),{scroll=true}={}){
  const ctx=context==='pro'?'pro':'config';presentationContext=ctx;
  if(ctx==='pro'){
    switchAdminTab('pro');
    if(window.quiz2ProBridge?.setPane)window.quiz2ProBridge.setPane('presentation',{remember:true,scroll:false});
    else{const panel=$('#adminPanelPro');if(panel){panel.dataset.proPane='presentation';$$('#proConfigSubtabs [data-pro-pane]').forEach(btn=>{const on=btn.dataset.proPane==='presentation';btn.classList.toggle('active',on);btn.setAttribute('aria-selected',String(on));});}}
    mountPresentation('pro');
  }else{switchAdminTab('config');setConfigPane('presentation',{scroll:false});mountPresentation('config');}
  if(scroll)setTimeout(()=>$('#sharedPresentationWorkspace')?.scrollIntoView({behavior:'smooth',block:'start'}),60);
}
function switchAdminTab(name){
  if(name==='presentation'){openPresentationContext(presentationContextForRoom());return;}
  const tab=['central','questions','config','pro'].includes(name)?name:'central';activeAdminTab=tab;document.documentElement.classList.toggle('questions-page-scroll',tab==='questions');document.body.classList.toggle('questions-page-scroll',tab==='questions');$$('.admin-tab').forEach(btn=>{const on=btn.dataset.tab===tab;btn.classList.toggle('active',on);btn.setAttribute('aria-selected',String(on));});$$('.tab-panel').forEach(panel=>panel.classList.toggle('active',panel.dataset.panel===tab));try{localStorage.setItem('quiz2AdminTab',tab);}catch{}if(tab==='config'&&$('#adminPanelConfig')?.dataset.configPane==='presentation')mountPresentation('config');if(tab==='config'&&$('#adminPanelConfig')?.dataset.configPane==='questions')mountQuestionBank('config');if(tab==='pro'&&$('#adminPanelPro')?.dataset.proPane==='presentation')mountPresentation('pro');if(tab==='pro'&&$('#adminPanelPro')?.dataset.proPane==='questions')mountQuestionBank('pro');if(tab==='central'){renderSetupReview();loadRecentRooms();}}
function initAdminTabs(){if($('#adminTabs')?.dataset.ready)return;$('#adminTabs').dataset.ready='1';$$('.admin-tab').forEach(btn=>btn.addEventListener('click',()=>switchAdminTab(btn.dataset.tab)));let saved='central';try{saved=localStorage.getItem('quiz2AdminTab')||'central';presentationContext=localStorage.getItem('quiz2PresentationContext')||'config';}catch{}if(saved==='presentation')saved=presentationContext==='pro'?'pro':'config';switchAdminTab(['central','questions','config','pro'].includes(saved)?saved:'central');}
function setQuestionPane(name,{scroll=false}={}){const panel=$('#adminPanelQuestions');if(!panel)return;const pane=['edit','import'].includes(name)?name:'edit';panel.dataset.questionPane=pane;[...panel.querySelectorAll('.question-subtabs [data-question-pane]')].forEach(btn=>{const on=btn.dataset.questionPane===pane;btn.classList.toggle('active',on);btn.setAttribute('aria-selected',String(on));});try{localStorage.setItem('quiz2QuestionPane',pane);}catch{}if(scroll)panel.querySelector('.question-subtabs')?.scrollIntoView({behavior:'smooth',block:'nearest'});}
...
function setConfigPane(name,{scroll=false}={}){const panel=$('#adminPanelConfig');if(!panel)return;const pane=['room','questions','rules','rounds','review','presentation'].includes(name)?name:'room';panel.dataset.configPane=pane;[...panel.querySelectorAll('.config-subtabs [data-config-pane]')].forEach(btn=>{if(!btn.matches('button'))return;const on=btn.dataset.configPane===pane;btn.classList.toggle('active',on);btn.setAttribute('aria-selected',String(on));});try{localStorage.setItem('quiz2ConfigPane',pane);}catch{}if(pane==='questions')mountQuestionBank('config');if(pane==='review')renderStandardReview();if(pane==='presentation')mountPresentation('config');if(scroll)panel.querySelector('.config-subtabs')?.scrollIntoView({behavior:'smooth',block:'nearest'});}
...
function jumpConfigBlock(name){
  if(name==='questions'){switchAdminTab('config');setConfigPane('questions',{scroll:true});return;}
...
function renderLobbyControls(){const s=roomSettings(),mode=String(s.lobby_map_mode||'auto'),scene=['office','laboratory','industry','platform'].includes(String(s.avatar_scene||'').toLowerCase())?String(s.avatar_scene).toLowerCase():'auto',sceneLabel={auto:'automático',office:'Escritório',laboratory:'Laboratório',industry:'Indústria',platform:'Plataforma'}[scene]||'automático',r=lobbyReadiness();$$('[data-lobby-layout]').forEach(b=>b.classList.toggle('active',b.dataset.lobbyLayout===mode));$$('[data-avatar-scene-live]').forEach(b=>b.classList.toggle('active',b.dataset.avatarSceneLive===scene));const badge=$('#lobbyReadinessAdmin');if(badge){badge.textContent=r.total?`${r.ready}/${r.total} prontos • ${r.online}/${r.total} online`:'0 prontos';badge.className=`badge ${r.highlight?'ready':'neutral'}`;}const sceneBadge=$('#presentationSceneStatus');if(sceneBadge){sceneBadge.textContent=`Mapa atual: ${sceneLabel}`;sceneBadge.className=`badge ${scene==='auto'?'neutral':'ready'}`;}if($('#startReadyCheck')){$('#startReadyCheck').textContent=r.total?`${r.ready}/${r.total} prontos • ${r.online}/${r.total} online`:'0 jogadores prontos • opcional';$('#startReadyCheck').className=`start-check ${r.highlight?'ready':'neutral'}`;}}
...
async function resetNewRoomQueueIfNeeded(reuse=false){
  if(!room)return 0;
  const{data,error}=await db.rpc('admin_list_room_queue',{p_room_id:room.id});if(error)throw error;
  const carried=Array.isArray(data)?data:[];
  if(reuse||!carried.length)return carried.length;
  for(const q of carried){const rm=await db.rpc('admin_remove_queue_item',{p_room_id:room.id,p_queue_id:q.id});if(rm.error)throw rm.error;}
  return 0;
}
async function createRoom(){
  const reuse=!!reuseQuestionsForNextRoom;
  if(room){const risk=roomRiskSummary();if(risk.hasWork){const carry=reuse&&risk.queued>0?` ${risk.queued} pergunta(s) serão reutilizadas.`:' A nova sala começará sem perguntas.';const msg=`A sala atual ${room.code} será encerrada ao criar a nova sala.${carry} Jogadores permanecem vinculados à sala anterior e podem entrar novamente pelo novo PIN.\n\nDeseja continuar?`;if(!confirm(msg))return;}}
  ...
  await withAction('Criando sala…',async()=>{
    const{data,error}=await db.rpc('admin_create_room_v4',{p_title:$('#roomTitle').value.trim()||'Quiz ao vivo',p_planned_rounds:plannedRounds()});if(error)throw error;
    room=data;rememberRoom();roomState=null;queueRows=[];presenceOnline=null;displayOnline=false;lastAdminQr='';eventInsights=null;lastInsightsRounds=-1;settingsDraftDirty=true;settingsDraftSerial++;$('#eventInsightsCard')?.classList.add('hidden');$('#plannedRounds').value=room.planned_rounds||plannedRounds();$('#roomInfo').textContent=`Código: ${room.code}`;$('#roomInfo').classList.add('room-protected');$('#roomProgress').classList.remove('hidden');
    await claimController(true);const kept=await resetNewRoomQueueIfNeeded(reuse);await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants()]);await saveRoomSettings(false);await subscribe();
    if($('#queueWarning')){$('#queueWarning').textContent=kept>0?`${kept} pergunta(s) reutilizada(s) da partida anterior.`:'Nova sala criada com a seleção de perguntas vazia.';$('#queueWarning').className='queue-warning ready';}
    reuseQuestionsForNextRoom=false;if($('#newGameReuseQuestions'))$('#newGameReuseQuestions').checked=false;
  },{skipControllerGuard:true});
}
async function createGameShowRoomFromPro(detail={}){
  const title=String(detail?.title||'Quiz GameShow Pro').trim().slice(0,120)||'Quiz GameShow Pro';
  const rounds=Math.max(1,Math.min(200,Number(detail?.plannedRounds)||10));
  const patch=detail?.settingsPatch&&typeof detail.settingsPatch==='object'?detail.settingsPatch:{};
  const teams=Array.isArray(detail?.teams)?detail.teams.slice(0,12):[];
  const reuse=!!reuseQuestionsForNextRoom;
  if(String(patch.game_mode||'')==='teams'&&backendSchema()<TEAM_FEATURE_SCHEMA_REQUIRED)throw new Error('Equipes com formação automática/jogador/ADM exigem schema 043. Execute 00_ATUALIZAR_SUPABASE_PARA_SCHEMA_043.sql.');
  if(room){const risk=roomRiskSummary();if(risk.hasWork){const carry=reuse&&risk.queued>0?` ${risk.queued} pergunta(s) serão reutilizadas.`:' O novo GameShow começará sem perguntas.';if(!confirm(`A sala atual ${room.code} será encerrada ao criar o novo GameShow.${carry}\n\nDeseja continuar?`))return null;}}
  let authorityReady=false;try{authorityReady=await ensureControllerForRoomCreation();}catch(e){diag('error','Falha ao preparar controlador para novo GameShow',{error:e.message||String(e)});throw e;}if(!authorityReady)return null;
  let created=null;
  const ok=await withAction('Criando GameShow…',async()=>{
    const{data,error}=await db.rpc('admin_create_room_v4',{p_title:title,p_planned_rounds:rounds});if(error)throw error;
    room=data;created=data;rememberRoom();roomState=null;queueRows=[];presenceOnline=null;displayOnline=false;lastAdminQr='';eventInsights=null;lastInsightsRounds=-1;settingsDraftDirty=false;settingsDraftSerial=0;
    $('#eventInsightsCard')?.classList.add('hidden');$('#plannedRounds').value=room.planned_rounds||rounds;$('#roomTitle').value=room.title||title;$('#roomInfo').textContent=`Código: ${room.code}`;$('#roomInfo').classList.add('room-protected');$('#roomProgress').classList.remove('hidden');
    await claimController(true);const kept=await resetNewRoomQueueIfNeeded(reuse);
    if(Object.keys(patch).length){const settingsResult=await db.rpc('admin_update_room_settings',{p_room_id:room.id,p_patch:patch});if(settingsResult.error)throw settingsResult.error;if(settingsResult.data)room=settingsResult.data;created=room;}
    if(teams.length>=2){const teamResult=await db.rpc('admin_save_teams',{p_room_id:room.id,p_teams:teams});if(teamResult.error)throw teamResult.error;const join=await db.rpc('admin_set_team_join_mode',{p_room_id:room.id,p_mode:String(patch.team_join_mode||'auto')});if(join.error)throw join.error;}
    await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants(),loadRecentRooms()]);await subscribe();
    if($('#queueWarning')){$('#queueWarning').textContent=kept>0?`${kept} pergunta(s) reutilizada(s) da partida anterior.`:'Novo GameShow criado com a seleção de perguntas vazia.';$('#queueWarning').className='queue-warning ready';}
    reuseQuestionsForNextRoom=false;if($('#newGameReuseQuestions'))$('#newGameReuseQuestions').checked=false;
    window.dispatchEvent(new CustomEvent('quiz2:room-changed',{detail:{id:room.id,code:room.code,source:'gameshow-pro'}}));
  },{skipControllerGuard:true});
  return ok?created:null;
}
window.quiz2AdminBridge={...(window.quiz2AdminBridge||{}),createGameShowRoom:createGameShowRoomFromPro,getRoom:()=>room,getQueue:()=>queueRows.slice(),getChecks:()=>setupChecks(),getBackendSchema:()=>backendSchema(),validateGameQuestions:(rows)=>validateGameQuestions(rows),switchTab:(name)=>switchAdminTab(name),setConfigPane:(name)=>setConfigPane(name),mountPresentation:(context)=>mountPresentation(context),mountQuestionBank:(context)=>mountQuestionBank(context),openPresentation:(context)=>openPresentationContext(context),getPresentationContext:()=>presentationContext,saveTemplate:()=>saveTemplate(),testGame:()=>launchGuidedTest(),setSimpleMode:(simple)=>applySimpleMode(simple),isSimpleMode:()=>uiSimpleMode};
...
function startQuestionEdit(id){
  switchAdminTab('questions');setQuestionPane('edit');
...
function gameSpecOf(q){let spec=q?.game_spec??q?.game_spec_snapshot??{};if(typeof spec==='string'){try{spec=JSON.parse(spec);}catch{spec={};}}return spec&&typeof spec==='object'?spec:{};}
function mediaUrlOf(q){return String(q?.media_url||q?.media_url_snapshot||gameSpecOf(q).media_url||'').trim();}
function gameQuestionIssue(q){
  const type=String(q?.game_type||q?.game_type_snapshot||'standard'),s=gameSpecOf(q),arr=(v)=>Array.isArray(v)?v:[],num=(v)=>Number.isFinite(Number(v));
  if(type==='standard')return'';
  if(type==='nearest'&&!num(s.target??q.correct_number))return'Informe o valor-alvo.';
  if(type==='precision'&&(!num(s.target??q.correct_number)||![s.gold,s.silver,s.bronze].every(num)))return'Informe alvo e as faixas ouro, prata e bronze.';
  if(type==='ordering'&&(arr(s.items).length<2||arr(s.correct_order).length<2))return'Defina pelo menos 2 itens e a ordem correta.';
  if(type==='matching'&&(arr(s.left_items).length<2||arr(s.right_items).length<2||arr(s.correct_matches).length<2))return'Defina pelo menos 2 pares e o gabarito.';
  if(type==='classification'&&(arr(s.items).length<2||arr(s.categories).length<2||arr(s.classification_answers).length<2))return'Defina itens, categorias e classificação correta.';
  if(type==='true_false_series'&&(arr(s.statements).length<2||arr(s.statement_answers).length<2))return'Defina pelo menos 2 afirmações e o gabarito V/F.';
  if(['hidden_image','zoom_mystery'].includes(type)&&!mediaUrlOf(q))return'Adicione uma imagem para esta mecânica.';
  if(type==='who_am_i'&&arr(s.clues).length<2)return'Adicione pelo menos 2 pistas.';
  if(type==='before_after'&&(!mediaUrlOf(q)||!String(s.after_url||'').trim()))return'Adicione as imagens de antes e depois.';
  if(type==='case_study'&&!String(s.scenario||s.instructions||q.prompt||'').trim())return'Defina o cenário do caso técnico.';
  if(type==='decision_tree'&&arr(s.outcomes).length<2)return'Defina pelo menos 2 caminhos/resultados.';
  if(type==='team_mission'&&(arr(s.options).length<2||arr(s.correct_choices).length<1))return'Defina opções e pelo menos uma resposta correta.';
  if(type==='bingo'&&(arr(s.cells).length<9||arr(s.winning_lines).length<1))return'O Bingo precisa de pelo menos 9 células e uma linha vencedora.';
  if(type==='wheel'&&arr(s.segments).length<2)return'A Roda precisa de pelo menos 2 segmentos.';
  if(type==='surprise'&&arr(s.boxes).length<2)return'A Caixa Surpresa precisa de pelo menos 2 caixas.';
  if(['crowd_prediction','live_poll','audience_choice','category_choice'].includes(type)&&arr(s.options).length<2)return'Defina pelo menos 2 opções.';
  return'';
}
function validateGameQuestions(rows=queueRows){const issues=[];for(const q of rows||[]){const issue=gameQuestionIssue(q);if(issue)issues.push({id:q.id||q.question_id,type:String(q.game_type||q.game_type_snapshot||'standard'),prompt:String(q.prompt||q.prompt_snapshot||'Pergunta'),issue});}return issues;}
function questionCompatibility(q){
  if(questionBankContext!=='pro')return{key:'compatible',label:'Compatível'};
  const issue=gameQuestionIssue(q);return issue?{key:'review',label:'Precisa configurar',detail:issue}:{key:'compatible',label:'Compatível'};
}
function renderQuestionBank(){
  const canQueue=canEditQueue()&&!queueAtLimit(),queueTargetLabel=questionBankContext==='pro'?'GameShow Pro':'Partida';visibleQuestionRows=sortQuestionRows(questionBank.filter(questionMatches));selectedQuestionIds=new Set([...selectedQuestionIds].filter(id=>questionBank.some(q=>String(q.id)===String(id))));updateQuestionBankSummary(visibleQuestionRows);refreshCategoryQuickFilters();updateSelectionToolbar();
  $('#questionList').innerHTML=visibleQuestionRows.map(q=>{const archived=!!q.archived,queued=queueRows.some(x=>x.question_id===q.id),recentBlocked=recentQuestionIds.has(q.id)&&Number(roomSettings().avoid_recent_games||0)>0,disabled=!canQueue||archived||queued||recentBlocked,selected=selectedQuestionIds.has(String(q.id)),typeLabel=q.question_type==='choice'?'Escolha':'Número',compat=questionCompatibility(q);return `<div class="question-item ${archived?'archived':''} ${selected?'selected':''}" data-id="${q.id}"><label class="question-select" title="Selecionar"><input type="checkbox" data-act="select" ${selected?'checked':''}></label><div class="question-card-main"><div class="question-card-top"><strong>${esc(q.prompt)}</strong><div class="question-card-tags"><span class="qtag ${q.question_type}">${typeLabel}</span><span class="qtag ${q.difficulty==='final'?'final':''}">${esc(difficultyLabel(q.difficulty))}</span><span class="qtag compatibility ${compat.key}">${compat.label}</span>${q.is_tiebreaker?'<span class="qtag tiebreaker">DESEMPATE</span>':''}</div></div><div class="question-card-meta"><span>${esc(q.category||'Geral')}</span><span>${q.score_enabled===false?'Sem pontos':`${q.points} pts`}</span><span>${q.time_limit_seconds}s</span>${q.speed_bonus_pct?`<span>+${q.speed_bonus_pct}% velocidade</span>`:''}<span>${q.use_count?`Usada ${q.use_count}x`:'Nunca usada'}</span>${q.accuracy_pct!=null?`<span>${Number(q.accuracy_pct).toFixed(1)}% acerto</span>`:''}${q.avg_response_ms!=null?`<span>${(Number(q.avg_response_ms)/1000).toFixed(1)}s média</span>`:''}${recentBlocked?'<span>Recente</span>':''}</div><div class="question-card-actions"><button class="queue-question" data-act="queue" ${disabled?'disabled':''}>${queued?'Já escolhida':archived?'Arquivada':recentBlocked?'Usada recentemente':`＋ ${queueTargetLabel}`}</button><button class="edit-question" data-act="edit">Editar</button><button class="ghost" data-act="duplicate">Duplicar</button><button class="ghost archive-question" data-act="archive">${archived?'Restaurar':'Arquivar'}</button></div></div></div>`;}).join('')||'<div class="empty">Nenhuma pergunta corresponde aos filtros atuais.</div>';
...
async function setAvatarSceneQuick(scene='auto'){
  if(!room)return alert('Crie ou abra uma sala primeiro.');
  const allowed=['auto','office','laboratory','industry','platform'];
  const next=allowed.includes(String(scene||'').toLowerCase())?String(scene).toLowerCase():'auto';
  if(!remoteModeActive){const reclaimed=await claimController(false);if(!reclaimed)return alert('Este dispositivo não é o controlador ativo da sala. Use “Assumir controle” antes de trocar o mapa.');}
  await withAction(`Trocando mapa para ${next==='auto'?'Automático':({'office':'Escritório','laboratory':'Laboratório','industry':'Indústria','platform':'Plataforma'}[next]||next)}…`,async()=>{
    const {error}=await db.rpc('admin_update_room_settings',{p_room_id:room.id,p_patch:{avatar_scene:next,pro_presentation_overlay:'none'}});
    if(error)throw error;
    if($('#avatarScene'))$('#avatarScene').value=next;
...
async function templateTeamSetup(){
  if(!room||String(roomSettings().game_mode||'')!=='teams')return null;
  if(backendSchema()<TEAM_FEATURE_SCHEMA_REQUIRED)return null;
  const{data,error}=await db.rpc('admin_list_teams',{p_room_id:room.id});if(error)throw error;
  return{join_mode:String(roomSettings().team_join_mode||'auto'),teams:(data||[]).map(t=>({name:t.name,color:t.color,icon:t.icon||'●'})).slice(0,12)};
}
async function restoreTemplateTeams(){
  const setup=room?.settings?.template_team_setup;if(!setup||String(roomSettings().game_mode||'')!=='teams')return;
  if(backendSchema()<TEAM_FEATURE_SCHEMA_REQUIRED){actionToast('Modelo carregado. Para recriar as equipes, atualize o Supabase para schema 043.','error');return;}
  const teams=Array.isArray(setup.teams)?setup.teams:[];if(teams.length>=2){const saved=await db.rpc('admin_save_teams',{p_room_id:room.id,p_teams:teams});if(saved.error)throw saved.error;const join=await db.rpc('admin_set_team_join_mode',{p_room_id:room.id,p_mode:String(setup.join_mode||'auto')});if(join.error)throw join.error;}
}
async function saveTemplate(){
  if(!room)return alert('Crie uma sala.');const name=prompt('Nome do modelo de partida:',`${room.title} — modelo`);if(!name)return;
  try{const teamSetup=await templateTeamSetup();if(String(roomSettings().game_mode||'')==='teams'&&backendSchema()<TEAM_FEATURE_SCHEMA_REQUIRED)throw new Error('Para salvar um modelo com equipes completas, atualize o Supabase para schema 043.');const patch=await db.rpc('admin_update_room_settings',{p_room_id:room.id,p_patch:{template_team_setup:teamSetup}});if(patch.error)throw patch.error;if(patch.data)room=patch.data;const{error}=await db.rpc('admin_save_template',{p_room_id:room.id,p_name:name});if(error)throw error;await loadTemplates();actionToast('Modelo salvo com perguntas, regras e equipes ✓');}catch(e){alert(e.message||String(e));}
}
async function deleteTemplate(){const id=$('#templateSelect').value;if(!id||!confirm('Excluir este modelo?'))return;const{error}=await db.rpc('admin_delete_template',{p_template_id:id});if(error)return alert(error.message);await loadTemplates();}
async function useTemplate(){const id=$('#templateSelect').value;if(!id)return alert('Selecione um modelo.');let authorityReady=false;try{authorityReady=await ensureControllerForRoomCreation();}catch(e){return alert(e.message||String(e));}if(!authorityReady)return;await withAction('Criando sala do modelo…',async()=>{const{data,error}=await db.rpc('admin_create_from_template',{p_template_id:id,p_title:null});if(error)throw error;room=data;rememberRoom();settingsDraftDirty=false;settingsDraftSerial=0;updateSettingsSaveIndicator('idle');roomState=null;queueRows=[];presenceOnline=null;displayOnline=false;lastAdminQr='';$('#plannedRounds').value=room.planned_rounds||10;await claimController(true);await restoreTemplateTeams();await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants(),loadQuestions()]);await subscribe();const ctx=presentationContextForRoom();if(ctx==='pro'){switchAdminTab('pro');window.quiz2ProBridge?.setPane?.('review',{remember:true,scroll:true});}else{switchAdminTab('config');setConfigPane('review',{scroll:true});}},{skipControllerGuard:true});}
async function duplicateRoom(){
  if(!room)return alert('Crie ou abra uma sala.');const title=prompt('Nome da cópia:',`${room.title} — cópia`);if(!title)return;const sourceTeamSetup=await templateTeamSetup().catch(()=>null);
  await withAction('Duplicando sala…',async()=>{const{data,error}=await db.rpc('admin_duplicate_room',{p_room_id:room.id,p_title:title});if(error)throw error;room=data;rememberRoom();roomState=null;queueRows=[];presenceOnline=null;displayOnline=false;lastAdminQr='';$('#plannedRounds').value=room.planned_rounds||10;await claimController(true);if(sourceTeamSetup&&backendSchema()>=TEAM_FEATURE_SCHEMA_REQUIRED){const saved=await db.rpc('admin_save_teams',{p_room_id:room.id,p_teams:sourceTeamSetup.teams||[]});if(saved.error)throw saved.error;const join=await db.rpc('admin_set_team_join_mode',{p_room_id:room.id,p_mode:String(sourceTeamSetup.join_mode||'auto')});if(join.error)throw join.error;}await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants(),loadQuestions()]);await subscribe();const ctx=presentationContextForRoom();if(ctx==='pro'){switchAdminTab('pro');window.quiz2ProBridge?.setPane?.('review',{remember:true,scroll:true});}else switchAdminTab('config');},{skipControllerGuard:true});
}

async function startQuiz(){
  if(startQuizPromise){actionToast('A verificação de início já está em andamento. Aguarde a conclusão.','neutral');return startQuizPromise;}
  const task=(async()=>{if(!room)return alert('Crie ou abra uma sala primeiro.');if(phase()!=='lobby')return alert('O quiz só pode ser iniciado enquanto a sala está no lobby.');await Promise.all([refreshState(true),loadQueue()]);const setup=setupChecks(),blocking=setup.filter(x=>!x.ok);if(blocking.length)return alert('Revisão bloqueou o início: '+blocking.map(x=>x.label).join(', ')+'.');const stability=await stabilizeBeforeStart();if(!stability.ok)return alert(`Início bloqueado: ${stability.reason||'a sala ainda não está pronta'}.`);if(stability.fallback&&!confirm('O Realtime está temporariamente indisponível. O Supabase respondeu e o modo normal pode iniciar em fallback por polling, porém as atualizações podem chegar com alguns segundos de atraso. Deseja iniciar mesmo assim?'))return;const pf=await runPreflight({silent:true});if(!pf.hard)return alert('Pré-teste bloqueou o início: corrija Supabase, perguntas, controlador e, no Modo Evento, o Realtime/telão.');if(!pf.all){const passiveOnly=(pf.issues||[]).every(x=>/inicializando|fora do estado exato|heartbeat|Realtime em fallback/i.test(String(x)));if(passiveOnly)diag('warn','Pré-teste possui apenas alertas passivos; início liberado sem reconectar aparelhos',{issues:pf.issues});else if(!confirm('O pré-teste ainda tem alerta operacional. Deseja iniciar mesmo assim?'))return;}const queued=queueRows.length;if(queued<1)return alert('Adicione pelo menos 1 pergunta à sala antes de começar.');const saved=await withAction('Validando configurações…',async()=>{await saveRoomSettings(false);});if(!saved)return;await withAction(`Preparando ${queued} pergunta${queued===1?'':'s'}…`,async()=>{const{error}=await db.rpc('admin_start_quiz',{p_room_id:room.id});if(error)throw error;await Promise.all([refreshState(true),loadQueue(),loadAudit()]);$('#plannedRounds').value=Number(roomState?.room?.planned_rounds||queued);});})();
  startQuizPromise=task;
  try{return await task;}finally{if(startQuizPromise===task)startQuizPromise=null;}
}
...
async function closeRound(){if(!room)return;await withAction('Encerrando e pontuando…',async()=>{const{data,error}=await db.rpc('admin_close_and_score_round_r47',{p_room_id:room.id});if(error)throw error;await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants(),loadHistory(),loadEventInsights()]);if(data?.finished)$('#roundHint').textContent='Último round encerrado. Use as etapas de revelação para apresentar o resultado final.';});}
async function pauseQuiz(){if(!room)return false;const isPaused=phase()==='paused';return withAction(isPaused?'Retomando…':'Pausando…',async()=>{const{error}=await db.rpc('admin_pause_quiz',{p_room_id:room.id,p_pause:!isPaused});if(error)throw error;await refreshState(true);});}
...
function joinUrl(){if(!room)return'';const url=new URL('index.html',location.href);url.search='';url.hash='';url.searchParams.set('code',room.code);url.searchParams.set('qr_build',BUILD_ID);return url.href;}
...
async function openDisplay(mode='reuse'){
  ...
  if(!backendCompatible)return alert(backendCheckStatus!=='incompatible'?'Não foi possível confirmar o backend. Confira a conexão com o Supabase antes de abrir o telão.':`Backend incompatível confirmado. Verifique o schema ${String(BACKEND_SCHEMA_REQUIRED).padStart(3,'0')} antes de abrir o telão.`);
  ...
  try{
    const overlayReset=await db.rpc('admin_update_room_settings',{p_room_id:room.id,p_patch:{pro_presentation_overlay:'none'}});
    if(overlayReset.error)throw new Error(`Não foi possível limpar a sobreposição do telão: ${overlayReset.error.message||'erro no Supabase'}`);
    if(overlayReset.data)room=overlayReset.data;
    const{data,error}=await db.rpc('admin_create_display_pairing',{p_room_id:room.id});if(error)throw error;
    const url=`display.html?code=${encodeURIComponent(room.code)}&pair=${encodeURIComponent(data?.pair_token||'')}&qr_build=${encodeURIComponent(BUILD_ID)}`;
...
function realtimeStateVersion(){return Number(roomState?.room?.state_version||0);}
async function syncStateToVersion(expected=0,{force=true}={}){const target=Math.max(0,Number(expected)||0);realtimeTargetVersion=Math.max(realtimeTargetVersion,target);if(realtimeSyncPromise)return realtimeSyncPromise;realtimeSyncPromise=(async()=>{const started=performance.now(),waits=[0,90,180,360,650,950];let converged=false;try{for(let i=0;i<waits.length&&performance.now()-started<2800;i++){if(waits[i])await sleep(waits[i]);await refreshState(force&&i===0);if(!realtimeTargetVersion||realtimeStateVersion()>=realtimeTargetVersion){realtimeTargetVersion=0;converged=true;break;}}if(!converged&&realtimeTargetVersion){diag('warn','Realtime não convergiu no ciclo rápido; usando polling de segurança',{target:realtimeTargetVersion,current:realtimeStateVersion()});realtimeTargetVersion=0;realtimeFallbackUntil=Date.now()+12000;setPoll(POLL_FALLBACK);}}finally{realtimeSyncPromise=null;}})();return realtimeSyncPromise;}
function scheduleRealtimeAuxRefresh(){clearTimeout(realtimeAuxTimer);realtimeAuxTimer=setTimeout(()=>{Promise.all([loadQueue(),loadAudit(),loadParticipants()]).catch(()=>{});},180);}
function jitterDelay(ms,spread=.22){return Math.max(700,Math.round(ms*(1+((Math.random()*2)-1)*spread)));}
function setPoll(ms){clearTimeout(poll);const tick=async()=>{if(!realtimeSyncPromise)await refreshState(false);const next=Date.now()<realtimeFallbackUntil?POLL_FALLBACK:ms;poll=setTimeout(tick,jitterDelay(next));};poll=setTimeout(tick,jitterDelay(ms));clearInterval(auditTick);auditTick=setInterval(()=>{if(!realtimeSyncPromise)Promise.all([loadAudit(),loadParticipants()]).catch(()=>{});},jitterDelay(Math.max(7000,ms*2),.15));}
...
function presenceDevice(meta={},fallbackKind='device',fallbackId=''){return{kind:String(meta.kind||fallbackKind),id:String(meta.participant_id||meta.display_id||meta.user_id||fallbackId||''),session_id:String(meta.session_id||''),reconnect_epoch:Number(meta.reconnect_epoch||0),build:String(meta.build||meta.version||'antiga'),state_version:Number(meta.state_version||0),generation:Number(meta.generation||0),phase:String(meta.phase||''),at:Number(meta.at||0)};}
// SYNC_CONTRACT_BEGIN
function connectionIdentity(row={}){const kind=String(row.kind||((row.display_id||row.kind==='display')?'display':'participant')),id=String(row.participant_id||row.display_id||row.user_id||row.id||'');return id?`${kind}:${id}`:'';}
function expectedConnectionTargets(devices=[]){const out=new Set();for(const d of devices||[]){const kind=String(d.kind||''),id=String(d.id||d.participant_id||d.display_id||'');if(id&&(kind==='participant'||kind==='display'))out.add(`${kind}:${id}`);}return[...out].sort();}
function exactPresenceIdentitySet(devices=[],targetVersion=0,targetGeneration=0,now=Date.now(),build=BUILD_ID){
  const latest=new Map();
  for(const d of devices||[]){
    const identity=connectionIdentity(d);
    if(!identity)continue;
    const prev=latest.get(identity);
    if(!prev||Number(d.at||0)>=Number(prev.at||0))latest.set(identity,d);
  }
  const exact=new Set();
  for(const [identity,d] of latest){
    const maxAge=d.kind==='display'?30000:36000;
    if(String(d.build||'')===String(build||'')&&Number(d.generation||0)===Number(targetGeneration||0)&&Number(d.state_version||0)===Number(targetVersion||0)&&Math.max(0,Number(now)-Number(d.at||0))<maxAge)exact.add(identity);
  }
  return exact;
}
function summarizePresenceLag(devices=[],currentVersion=0,currentGeneration=0,now=Date.now(),build=BUILD_ID){const dedup=new Map();for(const d of devices||[]){const key=`${d.kind}:${d.id||d.session_id||'unknown'}`,prev=dedup.get(key);if(!prev||Number(d.at||0)>=Number(prev.at||0))dedup.set(key,d);}const target=Number(currentVersion)||0,generation=Number(currentGeneration)||0,all=[...dedup.values()].map(d=>{const dv=Number(d.state_version||0),dg=Number(d.generation||0),sameGeneration=dg===generation&&dv>0,gap=sameGeneration?target-dv:Math.max(1,target||1);return{...d,age:Math.max(0,Number(now)-Number(d.at||0)),lag:Math.max(0,gap),aheadBy:Math.max(0,-gap),delta:sameGeneration?dv-target:null};}),sameBuild=all.filter(d=>String(d.build||'')===String(build||'')),foreignBuild=all.filter(d=>String(d.build||'')!==String(build||'')),fresh=sameBuild.filter(d=>d.age<(d.kind==='display'?30000:36000)),stale=sameBuild.filter(d=>d.age>=(d.kind==='display'?30000:36000)),initializing=fresh.filter(d=>Number(d.state_version||0)<=0||Number(d.generation||0)!==generation),ready=fresh.filter(d=>Number(d.state_version||0)>0&&Number(d.generation||0)===generation),synced=ready.filter(d=>Number(d.state_version||0)===target),converging=ready.filter(d=>Number(d.state_version||0)===target-1),hardLagging=ready.filter(d=>Number(d.state_version||0)<target-1),ahead=ready.filter(d=>Number(d.state_version||0)>target),outOfSync=[...initializing,...converging,...hardLagging,...ahead],displayLagging=outOfSync.filter(d=>d.kind==='display'),playerLagging=outOfSync.filter(d=>d.kind==='participant'),displayStale=stale.filter(d=>d.kind==='display'),playerStale=stale.filter(d=>d.kind==='participant');return{currentVersion:target,currentGeneration:generation,checked:fresh.length,known:sameBuild.length,foreignBuild:foreignBuild.length,synced:synced.length,initializing:initializing.length,converging:converging.length,hardLagging:hardLagging.length,ahead:ahead.length,lagging:outOfSync.length,worst:outOfSync.reduce((m,d)=>Math.max(m,Number(d.lag)||Number(d.aheadBy)||0),0),displayLagging:displayLagging.length,playerLagging:playerLagging.length,displayInitializing:initializing.filter(d=>d.kind==='display').length,playerInitializing:initializing.filter(d=>d.kind==='participant').length,displayAhead:ahead.filter(d=>d.kind==='display').length,playerAhead:ahead.filter(d=>d.kind==='participant').length,stale:stale.length,displayStale:displayStale.length,playerStale:playerStale.length,rows:outOfSync,staleRows:stale,initializingRows:initializing,convergingRows:converging,hardLaggingRows:hardLagging,aheadRows:ahead};}
function evaluateConnectionAcks(acks=[],expectedIdentities=[],targetVersion=0,targetGeneration=0,build=BUILD_ID){const expected=[...new Set((Array.isArray(expectedIdentities)?expectedIdentities:[]).map(String).filter(Boolean))],expectedSet=new Set(expected),latest=new Map();for(const row of Array.isArray(acks)?acks:[]){const key=connectionIdentity(row);if(!key)continue;const prev=latest.get(key);if(!prev||Number(row.at||0)>=Number(prev.at||0))latest.set(key,row);}const rows=[...latest.entries()].map(([identity,row])=>({...row,identity})),matched=rows.filter(a=>expectedSet.has(a.identity)),extras=rows.filter(a=>!expectedSet.has(a.identity)),respondedSet=new Set(matched.map(a=>a.identity)),missingIds=expected.filter(id=>!respondedSet.has(id)),sameBuild=matched.filter(a=>String(a.build||'')===String(build||'')),wrongBuild=matched.length-sameBuild.length,exact=sameBuild.filter(a=>Number(a.generation||0)===Number(targetGeneration)&&Number(a.state_version||0)===Number(targetVersion)),initializing=sameBuild.filter(a=>Number(a.state_version||0)<=0||Number(a.generation||0)!==Number(targetGeneration)),lagging=sameBuild.filter(a=>Number(a.generation||0)===Number(targetGeneration)&&Number(a.state_version||0)>0&&Number(a.state_version||0)<Number(targetVersion)),ahead=sameBuild.filter(a=>Number(a.generation||0)===Number(targetGeneration)&&Number(a.state_version||0)>Number(targetVersion)),complete=missingIds.length===0&&wrongBuild===0&&initializing.length===0&&lagging.length===0&&ahead.length===0&&exact.length===expected.length;return{complete,responders:matched.length,received:rows.length,expected:expected.length,synced:exact.length,missing:missingIds.length,missingIds,extra:extras.length,extraIds:extras.map(a=>a.identity),initializing:initializing.length,lagging:lagging.length,ahead:ahead.length,wrongBuild,targetVersion:Number(targetVersion)||0,targetGeneration:Number(targetGeneration)||0};}
// SYNC_CONTRACT_END
function presenceLagSummary(){const currentVersion=Number(roomState?.room?.state_version||0),currentGeneration=Number(roomState?.room?.generation||0),raw=[...(presenceDevices.players||[]),...(presenceDevices.displays||[])];return summarizePresenceLag(raw,currentVersion,currentGeneration,Date.now(),BUILD_ID);}
async function trackAdminPresence(){if(!room||!channel)return false;try{const result=await channel.track({kind:'admin',version:BUILD_ID,build:BUILD_ID,device_label:controllerDeviceLabel(),state_version:Number(roomState?.room?.state_version||0),generation:Number(roomState?.room?.generation||room?.generation||1),phase:roomState?.room?.phase||room?.phase||'lobby',at:Date.now()});return !result||result==='ok';}catch{return false;}}
async function runConnectionTest({silent=false,timeoutMs=6000}={}){
  if(connectionTestPromise)return connectionTestPromise;
  const task=(async()=>{
    if(!room||!channel){
      if(!silent)alert('Abra uma sala e aguarde a conexão.');
      return{complete:false,responders:0,expected:0,synced:0,missing:0,initializing:0,lagging:0,ahead:0,wrongBuild:0,error:'sem canal'};
    }
    const nonce=`ct-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    connectionTestNonce=nonce;
    connectionAcks=new Map();
    connectionExpectedTargets=expectedConnectionTargets([...(presenceDevices.players||[]),...(presenceDevices.displays||[])]);
    const out=$('#connectionTestResult'),targetVersion=Number(roomState?.room?.state_version||0),targetGeneration=Number(roomState?.room?.generation||1),deadline=Math.max(2500,Math.min(8000,Number(timeoutMs)||6000));
    if(out){out.textContent=`Testando ${connectionExpectedTargets.length} aparelho(s) por identidade…`;out.className='badge neutral';}
    try{
      await channel.send({type:'broadcast',event:'connection_test',payload:{nonce,state_version:targetVersion,generation:targetGeneration,expected:connectionExpectedTargets,deadline_ms:deadline,at:Date.now()}});
    }catch(e){
      if(connectionTestNonce===nonce)connectionTestNonce='';
      connectionExpectedTargets=[];
      if(out){out.textContent='Falha ao enviar teste';out.className='badge warn';}
      return{complete:false,responders:0,expected:connectionExpectedTargets.length,synced:0,missing:connectionExpectedTargets.length,initializing:0,lagging:0,ahead:0,wrongBuild:0,error:e?.message||String(e),targetVersion,targetGeneration};
    }
    return await new Promise(resolve=>{
      let done=false;
      const expectedSnapshot=[...connectionExpectedTargets];
      const finish=()=>{
        if(done)return;
        done=true;
        if(connectionTestTimer){clearTimeout(connectionTestTimer);connectionTestTimer=null;}
        if(connectionTestNonce!==nonce){
          resolve({complete:false,cancelled:true,responders:0,expected:expectedSnapshot.length,synced:0,missing:expectedSnapshot.length,missingIds:expectedSnapshot,initializing:0,lagging:0,ahead:0,wrongBuild:0,targetVersion,targetGeneration});
          return;
        }
        const result=evaluateConnectionAcks([...connectionAcks.values()],expectedSnapshot,targetVersion,targetGeneration,BUILD_ID);
        if(out){
          const extras=[];
          if(result.missing)extras.push(`${result.missing} identidade${result.missing===1?'':'s'} sem ACK`);
          if(result.initializing)extras.push(`${result.initializing} inicializando`);
          if(result.lagging)extras.push(`${result.lagging} atrasado${result.lagging===1?'':'s'}`);
          if(result.ahead)extras.push(`${result.ahead} à frente/divergente${result.ahead===1?'':'s'}`);
          if(result.wrongBuild)extras.push(`${result.wrongBuild} build diferente`);
          if(result.extra)extras.push(`${result.extra} resposta${result.extra===1?' extra':'s extras'} ignorada${result.extra===1?'':'s'}`);
          out.textContent=`${result.responders}/${result.expected} ACKs • ${result.synced} no estado exato${extras.length?' • '+extras.join(' • '):''}`;
          out.className=`badge ${result.complete?'ready':result.responders?'warn':'neutral'}`;
        }
        diag(result.complete?'test':'warn',result.complete?'Teste de sincronização por identidade aprovado':'ACK incompleto; presença Realtime será usada como confirmação secundária',{...result,missingIds:result.missingIds});
        connectionTestNonce='';
        connectionTestFinish=null;
        connectionExpectedTargets=[];
        resolve(result);
      };
      connectionTestFinish=finish;
      connectionTestTimer=setTimeout(finish,deadline);
      if(expectedSnapshot.length===0)setTimeout(finish,0);
    });
  })();
  connectionTestPromise=task;
  try{return await task;}finally{if(connectionTestPromise===task)connectionTestPromise=null;}
}
function syncSnapshot(){const lag=presenceLagSummary(),realtimeOk=lastConnectionKey.startsWith('connected');return{...lag,realtimeOk,versionMismatch:!!versionMismatch,displayOnline:!!displayOnline,ok:realtimeOk&&!versionMismatch&&lag.lagging===0&&lag.stale===0};}
async function broadcastSyncNudge(reason='manual'){if(!room||!channel||!lastConnectionKey.startsWith('connected'))return false;const payload={state_version:Number(roomState?.room?.state_version||0),generation:Number(roomState?.room?.generation||1),source:'admin-sync-recovery',reason,at:Date.now()};try{await channel.send({type:'broadcast',event:'state_changed',payload});diag('sync','Solicitação de convergência enviada',{reason,state_version:payload.state_version,generation:payload.generation});return true;}catch(e){diag('warn','Falha ao solicitar convergência',{reason,error:e?.message||String(e)});return false;}}
function maybeAutoRecoverLag(lag=presenceLagSummary()){if(!lag.lagging||autoRecoveryInFlight||!lastConnectionKey.startsWith('connected')||Date.now()-lastAutoRecoveryAt<10000)return;lastAutoRecoveryAt=Date.now();autoRecoveryInFlight=true;(async()=>{await broadcastSyncNudge('auto');await sleep(1200);await refreshState(false);await trackAdminPresence();const after=presenceLagSummary();if(after.lagging<lag.lagging)diag('sync','Auto-recuperação reduziu aparelhos atrasados',{before:lag.lagging,after:after.lagging});else if(after.lagging)diag('warn','Auto-recuperação aguardando convergência',{count:after.lagging,worst:after.worst});renderEventHealth();})().finally(()=>{autoRecoveryInFlight=false;});}
async function recoverEventSync({silent=false,timeoutMs=5200}={}){
  if(!room||!channel||!lastConnectionKey.startsWith('connected')){
    const result={ok:false,reason:'Realtime não está conectado',snapshot:syncSnapshot(),test:null};
    if(!silent)actionToast('Realtime ainda não está conectado.','error');
    return result;
  }
  const started=performance.now();
  let retried=false;
  await broadcastSyncNudge(silent?'prestart':'manual');
  let snap=syncSnapshot();
  while(performance.now()-started<Math.max(1800,Number(timeoutMs)||5200)){
    await sleep(650);
    await refreshState(false);
    await trackAdminPresence();
    snap=syncSnapshot();
    if(snap.ok)break;
    if(!retried&&performance.now()-started>2200){retried=true;await broadcastSyncNudge('retry');}
  }
  const test=await runConnectionTest({silent:true,timeoutMs:6000});
  const final=syncSnapshot();
  const sameTarget=Number(test?.targetVersion||0)===Number(final.currentVersion||0)&&Number(test?.targetGeneration||0)===Number(final.currentGeneration||0);
  const testExact=!!test?.complete&&sameTarget&&Number(test.responders||0)>=Number(test.expected||0)&&Number(test.synced||0)===Number(test.responders||0);
  const exactPresence=exactPresenceIdentitySet([...(presenceDevices.players||[]),...(presenceDevices.displays||[])],final.currentVersion,final.currentGeneration,Date.now(),BUILD_ID);
  const missingIds=Array.isArray(test?.missingIds)?test.missingIds:[];
  const missingCoveredByPresence=missingIds.every(id=>exactPresence.has(id));
  const ackConflict=Number(test?.wrongBuild||0)>0||Number(test?.initializing||0)>0||Number(test?.lagging||0)>0||Number(test?.ahead||0)>0;
  const presenceFallback=final.ok&&sameTarget&&!ackConflict&&missingCoveredByPresence;
  const ok=final.ok&&(testExact||presenceFallback);
  let reason='';
  if(!final.realtimeOk)reason='Realtime desconectado';
  else if(final.versionMismatch)reason='há versões diferentes conectadas';
  else if(final.initializing)reason=`${final.initializing} aparelho${final.initializing===1?'':'s'} ainda inicializando`;
  else if(final.converging)reason=`${final.converging} aparelho${final.converging===1?'':'s'} ainda um estado atrás`;
  else if(final.ahead)reason=`${final.ahead} aparelho${final.ahead===1?' está':'s estão'} em state_version à frente/divergente`;
  else if(final.hardLagging)reason=`${final.hardLagging} aparelho${final.hardLagging===1?'':'s'} ainda atrasado${final.hardLagging===1?'':'s'}`;
  else if(final.stale)reason=`${final.stale} aparelho${final.stale===1?'':'s'} sem heartbeat recente`;
  else if(!sameTarget)reason='o estado mudou durante o teste; a verificação será refeita no próximo comando';
  else if(ackConflict)reason='um ACK confirmou estado/build divergente';
  else if(test?.missing&&!missingCoveredByPresence)reason=`${test.missing} aparelho${test.missing===1?' não possui confirmação recente':'s não possuem confirmação recente'} nem por ACK nem por presença`;
  else if(!testExact&&!presenceFallback)reason='não foi possível confirmar o estado atual dos aparelhos';
  const result={ok,reason,snapshot:final,test,sameTarget,ackFallback:presenceFallback&&!testExact,presenceConfirmed:missingIds.filter(id=>exactPresence.has(id))};
  if(!silent){
    if(ok&&result.ackFallback)actionToast('Sincronização confirmada pela presença Realtime; alguns ACKs de teste não chegaram.','ok');
    else actionToast(ok?'Aparelhos sincronizados no estado exato.':`Sincronização ainda pendente${reason?': '+reason:''}.`,ok?'ok':'error');
  }
  if(ok&&result.ackFallback)diag('sync','ACK parcial aceito com presença Realtime exata e recente',{missing:test?.missing||0,presenceConfirmed:result.presenceConfirmed.length,state_version:final.currentVersion});
  renderEventHealth();
  return result;
}
// INICIO_PASSIVO_SEM_RECONNECT_BEGIN
async function stabilizeBeforeStart(){
  if(startStabilityPromise)return startStabilityPromise;
  const btn=$('#startQuizBtn'),old=btn?.textContent||'';
  const task=(async()=>{
    startStabilityBusy=true;
    if(btn){btn.disabled=true;btn.textContent='Verificando sala…';}
    try{
      // O início NÃO força state_changed, connection_test, subscribe/reconnect nem recuperação ativa.
      // A presença já publicada pelos aparelhos é somente observada. A convergência ocorre após
      // a mudança real de estado produzida por admin_start_quiz.
      const stateOk=await refreshState(false);
      await trackAdminPresence().catch(()=>false);
      const snap=syncSnapshot(),lag=presenceLagSummary(),realtimeOk=lastConnectionKey.startsWith('connected');
      if(!stateOk||!roomState){
        const result={ok:false,fallback:false,passive:true,reason:'não foi possível confirmar o estado atual da sala'};
        diag('warn','Início bloqueado: estado da sala indisponível',{passive:true});
        return result;
      }
      if(versionMismatch){
        const result={ok:false,fallback:false,passive:true,reason:'há aparelhos com build diferente'};
        diag('warn','Início bloqueado: builds diferentes conectados',{passive:true});
        return result;
      }
      if(eventModeActive&&!realtimeOk){
        const result={ok:false,fallback:false,passive:true,reason:'Modo Evento exige Realtime conectado'};
        diag('warn','Início bloqueado: Realtime obrigatório no Modo Evento',{passive:true});
        return result;
      }
      if(eventModeActive&&!displayOnline){
        const result={ok:false,fallback:false,passive:true,reason:'Modo Evento exige telão conectado'};
        diag('warn','Início bloqueado: telão obrigatório no Modo Evento',{passive:true});
        return result;
      }
      const fallback=!realtimeOk;
      if(lag.lagging||lag.stale||lag.initializing){
        diag('warn','Início seguirá sem forçar reconexão; aparelhos convergem na mudança real de estado',{passive:true,state_version:lag.currentVersion,lagging:lag.lagging,stale:lag.stale,initializing:lag.initializing});
      }else{
        diag('ok','Checagem passiva de início aprovada sem reconectar aparelhos',{passive:true,state_version:lag.currentVersion});
      }
      return{ok:true,fallback,passive:true,reason:fallback?'Realtime indisponível; início seguirá pelo fallback por polling':'',snapshot:snap};
    }finally{
      startStabilityBusy=false;
      if(btn){btn.disabled=false;btn.textContent=old||'Começar Quiz';}
      updateControls();
    }
  })();
  startStabilityPromise=task;
  try{return await task;}finally{if(startStabilityPromise===task)startStabilityPromise=null;}
}
// INICIO_PASSIVO_SEM_RECONNECT_END
function watchdogSnapshot(){const lag=presenceLagSummary(),realtimeOk=lastConnectionKey.startsWith('connected'),displayFresh=!!displayOnline&&lag.displayStale===0,active=['preparing','question_open','paused','result'].includes(phase());return{lag,realtimeOk,displayFresh,active,buildOk:!versionMismatch,online:navigator.onLine};}
function startEventWatchdog(){clearInterval(eventWatchdogTimer);eventWatchdogTimer=null;eventWatchdogTick();eventWatchdogTimer=setInterval(eventWatchdogTick,5000);}
async function eventWatchdogTick(){if(!room){lastWatchdogIssue='';return;}const snap=watchdogSnapshot(),issues=[];if(!snap.online)issues.push('sem internet');else if(!snap.realtimeOk)issues.push('Realtime reconectando');if(!snap.buildOk)issues.push('builds diferentes');if(snap.lag.initializing)issues.push(`${snap.lag.initializing} inicializando`);else if(snap.lag.lagging)issues.push(`${snap.lag.lagging} fora do estado exato`);if(snap.lag.displayStale)issues.push('telão sem heartbeat');if(snap.lag.playerStale)issues.push(`${snap.lag.playerStale} jogador${snap.lag.playerStale===1?'':'es'} sem heartbeat`);if(snap.active&&!displayOnline)issues.push('telão desconectado durante a partida');const key=issues.join('|');if(key!==lastWatchdogIssue){if(key)diag('warn','Watchdog detectou condição degradada',{issues,state_version:snap.lag.currentVersion});else if(lastWatchdogIssue)diag('ok','Watchdog: operação voltou ao normal',{state_version:snap.lag.currentVersion});lastWatchdogIssue=key;}if(snap.realtimeOk&&snap.lag.displayStale&&Date.now()-lastAutoRecoveryAt>12000){lastAutoRecoveryAt=Date.now();broadcastSyncNudge('watchdog-display').catch(()=>{});}if(phase()==='lobby'&&Date.now()-lastAutoPreflightAt>20000&&!preflightBusy){lastAutoPreflightAt=Date.now();runPreflight({silent:true,quick:true}).catch(()=>{});}renderEventHealth();}
async function subscribe(){
  ...
  progress.subscribe(status=>{
    ...
    if(['CHANNEL_ERROR','TIMED_OUT','CLOSED'].includes(status))scheduleReconnect(epoch);
  });
  const main=db.channel(`quiz:${room.id}`,{config:{private:true,presence:{key:`admin-${controllerDeviceToken()}`}}})
    .on('broadcast',{event:'state_changed'},async({payload})=>{if(epoch!==subscriptionEpoch)return;await syncStateToVersion(payload?.state_version,{force:true});scheduleRealtimeAuxRefresh();})
    .on('broadcast',{event:'connection_ack'},({payload})=>{if(epoch!==subscriptionEpoch)return;if(connectionTestNonce&&payload?.nonce===connectionTestNonce){const identity=connectionIdentity(payload||{});if(identity)connectionAcks.set(identity,payload||{});const expectedSet=new Set(connectionExpectedTargets),matched=[...connectionAcks.keys()].filter(k=>expectedSet.has(k)).length,el=$('#connectionTestResult');if(el)el.textContent=`${matched}/${connectionExpectedTargets.length} identidade(s) confirmada(s)`;if(connectionTestFinish&&connectionExpectedTargets.length>0&&matched>=connectionExpectedTargets.length)connectionTestFinish(false);}})
    .on('presence',{event:'sync'},()=>{if(epoch!==subscriptionEpoch||main!==channel)return;const state=main.presenceState()||{},ps=Object.values(state).flat(),playerMap=new Map(),players=[],displays=[],admins=[];for(const [presenceKey,metas] of Object.entries(state)){for(const meta of Array.isArray(metas)?metas:[]){if(meta?.kind==='participant'||meta?.participant_id){const d=presenceDevice(meta,'participant',presenceKey);players.push(d);playerMap.set(d.id||String(presenceKey),d.build);}else if(meta?.kind==='display'||meta?.display_id)displays.push(presenceDevice(meta,'display',presenceKey));else if(meta?.kind==='admin')admins.push(presenceDevice(meta,'admin',presenceKey));}}presenceOnline=playerMap.size;displayOnline=displays.length>0;presenceDevices={players,displays,admins};presenceVersions={players:[...playerMap.values()],displays:displays.map(x=>x.build),admins:admins.map(x=>x.build)};versionMismatch=[...presenceVersions.players,...presenceVersions.displays,...presenceVersions.admins].some(v=>String(v)!==BUILD_ID);if(versionMismatch&&lastVersionBroadcast!==BUILD_ID){lastVersionBroadcast=BUILD_ID;main.send({type:'broadcast',event:'version_required',payload:{version:APP_VERSION,build:BUILD_ID}}).catch(()=>{});}if(!versionMismatch)lastVersionBroadcast='';const lag=presenceLagSummary();if(lag.lagging!==lastPresenceLagCount){if(lag.lagging>0)diag('warn','Aparelhos com estado atrasado detectados',{count:lag.lagging,worst:lag.worst,currentVersion:lag.currentVersion});else if(lastPresenceLagCount>0)diag('state','Aparelhos voltaram a sincronizar',{currentVersion:lag.currentVersion});lastPresenceLagCount=lag.lagging;}maybeAutoRecoverLag(lag);updateLiveStats(roomState);renderRemoteControl();renderEventHealth();});
  channel=main;
  main.subscribe(async status=>{
    if(epoch!==subscriptionEpoch||main!==channel)return;
    if(status==='SUBSCRIBED'){
      clearTimeout(reconnectTimer);reconnectTimer=null;reconnectAttempt=0;conn('connected');setPoll(POLL_CONNECTED);await refreshState(true);await trackAdminPresence();return;
    }
    if(['CHANNEL_ERROR','TIMED_OUT','CLOSED'].includes(status))scheduleReconnect(epoch);
  });
}
...
function applyEventMode(){document.body.classList.toggle('event-mode',eventModeActive);const b=$('#eventModeBtn');if(b)b.textContent=eventModeActive?'Sair do modo Evento':'Modo Evento';if(eventModeActive)openPresentationContext(presentationContextForRoom(),{scroll:false});}
...
function renderEventHealth(){
  const supabaseOk=!!roomState&&lastLatency>=0,realtimeOk=lastConnectionKey.startsWith('connected'),players=Number(presenceOnline??roomState?.active_count??0),questions=Number(roomState?.queued_count??queueRows.filter(q=>q.status==='queued').length),lag=presenceLagSummary(),displayFresh=!!displayOnline&&lag.displayStale===0;
  ...
  const syncDetail=lag.initializing?`${lag.initializing} inicializando`:lag.ahead?`${lag.ahead} à frente/divergente`:lag.converging?`${lag.converging} um estado atrás`:lag.hardLagging?`${lag.hardLagging} atrasado${lag.hardLagging===1?'':'s'}`:lag.stale?`${lag.stale} heartbeat pendente${lag.stale===1?'':'s'}`:'Sincronizado';
  setHealthTile($('#healthRealtime'),realtimeOk?(lag.lagging||lag.stale?'warn':'ready'):navigator.onLine?'warn':'fail',realtimeOk?`Conectado • ${syncDetail}`:navigator.onLine?'Reconectando • fallback por polling':'Sem internet',lag.lagging?`Estado alvo v${lag.currentVersion} • pior atraso ${lag.worst}`:'Sincronização ao vivo • watchdog 5 s');
  setHealthTile($('#healthSync'),!realtimeOk?'warn':versionMismatch?'fail':lag.displayStale?'warn':lag.lagging?'warn':'ready',!realtimeOk?'Fallback por polling':versionMismatch?'Versões diferentes':lag.displayStale?'Telão sem heartbeat':lag.initializing?`${lag.initializing} inicializando`:lag.ahead?`${lag.ahead} divergente${lag.ahead===1?'':'s'}`:lag.converging?`${lag.converging} convergindo`:lag.hardLagging?`${lag.hardLagging} atrasado${lag.hardLagging===1?'':'s'}`:'Em dia',lag.lagging?`Exige state_version exato v${lag.currentVersion}`:lag.stale?`${lag.stale} presença${lag.stale===1?'':'s'} precisa${lag.stale===1?'':'m'} renovar`:`Estado v${lag.currentVersion} consistente`);
  setHealthTile($('#healthDisplay'),displayFresh?(lag.displayLagging?'warn':'ready'):displayOnline?'fail':'warn',displayFresh?(lag.displayInitializing?'Conectado • inicializando':lag.displayLagging?'Conectado • convergindo':'Conectado'):displayOnline?'Sem heartbeat':'Não detectado',lag.displayStale?'Watchdog aguardando renovação da presença':lag.displayLagging?'Telão precisa alcançar exatamente o estado atual':'Projetor / telão');
  setHealthTile($('#healthPlayers'),players>0?(lag.playerStale||lag.playerLagging?'warn':'ready'):'warn',`${players} conectado${players===1?'':'s'}`,lag.playerInitializing?`${lag.playerInitializing} jogador${lag.playerInitializing===1?'':'es'} inicializando`:lag.playerStale?`${lag.playerStale} presença${lag.playerStale===1?'':'s'} sem heartbeat recente`:lag.playerLagging?`${lag.playerLagging} jogador${lag.playerLagging===1?'':'es'} convergindo`:`${Number(roomState?.ready_count||0)} pronto(s) • ${Number(roomState?.participant_count||0)} cadastrado(s)`);
...
function backendSchema(){return Number(backendMeta?.schema_version||0);}
function renderBackendFeatureState(){
  const banner=$('#backendFeatureBanner'),badge=$('#backendFeatureBadge'),title=$('#backendFeatureTitle'),text=$('#backendFeatureText');if(!banner)return;
  const schema=backendSchema();banner.classList.remove('hidden','ready','warn','fail');
  if(backendCheckStatus==='unknown'){banner.classList.add('warn');if(title)title.textContent='Verificando backend';if(text)text.textContent='Aguardando confirmação do schema pelo Supabase.';if(badge){badge.textContent='VERIFICANDO';badge.className='badge neutral';}}
  else if(backendCheckStatus==='unavailable'){banner.classList.add('warn');if(title)title.textContent='Backend não verificado';if(text)text.textContent='Não foi possível consultar o Supabase agora. Verifique a conexão e tente novamente; nenhuma migration é recomendada sem confirmar o schema.';if(badge){badge.textContent='SEM RESPOSTA';badge.className='badge warn';}}
  else if(schema>=TEAM_FEATURE_SCHEMA_REQUIRED){banner.classList.add('ready');if(title)title.textContent='Backend completo';if(text)text.textContent='Todos os recursos, inclusive os 3 modos de formação de equipes, estão disponíveis.';if(badge){badge.textContent=`SCHEMA ${String(schema).padStart(3,'0')}`;badge.className='badge ready';}}
  else if(schema>=BACKEND_SCHEMA_REQUIRED){banner.classList.add('warn');if(title)title.textContent='Backend compatível — equipes avançadas opcionais';if(text)text.textContent='Partida Padrão e GameShow individual funcionam. Para Automático/Jogador/ADM em equipes, execute 00_ATUALIZAR_SUPABASE_PARA_SCHEMA_043.sql.';if(badge){badge.textContent=`SCHEMA ${String(schema).padStart(3,'0')}`;badge.className='badge warn';}}
  else{banner.classList.add('fail');if(title)title.textContent='Backend precisa ser atualizado';if(text)text.textContent=`Este frontend exige no mínimo o schema ${String(BACKEND_SCHEMA_REQUIRED).padStart(3,'0')}.`;if(badge){badge.textContent=`SCHEMA ${String(schema||0).padStart(3,'0')}`;badge.className='badge warn';}}
}
async function checkBackendCompatibility({announce=false}={}){
  ...
  try{
    ...
    if(error){backendCompatible=false;backendMeta=null;backendCheckStatus='unavailable';renderBackendFeatureState();diag('warn','Supabase não respondeu à verificação do backend',{error:error.message||String(error)});if(announce)alert('Não foi possível verificar o backend agora. Confira a conexão com o Supabase e tente novamente. Nenhuma migration foi indicada.');return{ok:false,error,unavailable:true};}
    ...
    backendMeta=data||null;backendCompatible=schema>=BACKEND_SCHEMA_REQUIRED&&sameRelease&&buildOk;backendCheckStatus=backendCompatible?'ok':'incompatible';renderBackendFeatureState();
    if(!backendCompatible){const reason=`schema ${schema||0}; requerido ${BACKEND_SCHEMA_REQUIRED}; build mínimo ${requiredBuild||'não informado'}`;diag('error','Backend incompatível com esta versão',{reason,build:BUILD_ID});if(announce)alert(`QuizRounds ${BUILD_ID}: backend incompatível confirmado (schema ${schema||0}). Atualize o Supabase somente se o schema realmente estiver abaixo de ${BACKEND_SCHEMA_REQUIRED}.`);}
    else{diag(schema>=TEAM_FEATURE_SCHEMA_REQUIRED?'ok':'warn',schema>=TEAM_FEATURE_SCHEMA_REQUIRED?'Backend completo':'Backend base compatível; equipes avançadas requerem schema 043',{schema,release:data?.release||'',build:BUILD_ID});}
    return{ok:backendCompatible,data,error:null};
  }catch(error){backendCompatible=false;backendMeta=null;backendCheckStatus='unavailable';renderBackendFeatureState();diag('warn','Não foi possível verificar a versão do backend',{error:error.message||String(error)});if(announce)alert('Não foi possível verificar o backend agora. Confira sua conexão e tente novamente; não aplique migration por causa deste erro de comunicação.');return{ok:false,error,unavailable:true};}
}
async function runPreflight({silent=false,quick=false}={}){
  if(preflightBusy)return lastPreflight||{all:false,hard:false,issues:['pré-teste em andamento']};
  if(!room){lastPreflight={all:false,hard:false,issues:['sala'],checked_at:Date.now()};if(!silent)alert('Crie uma sala antes do pré-teste.');return lastPreflight;}
  preflightBusy=true;const summary=$('#preflightSummary');if(summary&&!quick){summary.className='preflight-summary neutral';summary.textContent='Executando pré-teste…';}
  try{
    let backend={ok:backendCompatible,data:backendMeta||null},rpcOk=false,lat=lastLatency;
    if(!quick||backendCheckStatus==='unknown'||Date.now()-lastBackendCheckAt>120000)backend=await checkBackendCompatibility();
    try{const t0=performance.now();const ok=await refreshState(false);lat=Math.round(performance.now()-t0);rpcOk=!!ok&&!!roomState;if(rpcOk){lastLatency=lat;await trackAdminPresence();}}catch{rpcOk=false;}
    const backendOk=!!backend.ok,realtimeOk=lastConnectionKey.startsWith('connected'),lag=presenceLagSummary(),displayFresh=!!displayOnline&&lag.displayStale===0,questionsOk=Number(roomState?.queued_count??queueRows.filter(q=>q.status==='queued').length)>0,controllerOk=controllerGranted||remoteModeActive,versionsOk=!versionMismatch,syncOk=lag.lagging===0,heartbeatOk=lag.playerStale===0,issues=[];
    if(!backendOk)issues.push(backendCheckStatus==='incompatible'?`backend schema ${BACKEND_SCHEMA_REQUIRED}`:'backend não verificado');if(!rpcOk)issues.push('Supabase');if(!realtimeOk)issues.push(eventModeActive?'Realtime obrigatório':'Realtime em fallback');if(!displayOnline)issues.push('telão não detectado');else if(!displayFresh)issues.push('telão sem heartbeat');if(!questionsOk)issues.push('perguntas');if(!controllerOk)issues.push('controlador');if(!versionsOk)issues.push('versões diferentes');if(lag.initializing)issues.push(`${lag.initializing} inicializando`);else if(!syncOk)issues.push(`${lag.lagging} aparelho${lag.lagging===1?'':'s'} fora do estado exato`);if(!heartbeatOk)issues.push(`${lag.playerStale} jogador${lag.playerStale===1?'':'es'} sem heartbeat`);
    const displayRequired=eventModeActive,realtimeRequired=eventModeActive,transportOk=realtimeOk||(!realtimeRequired&&rpcOk),hard=backendOk&&rpcOk&&transportOk&&questionsOk&&controllerOk&&versionsOk&&(!displayRequired||displayFresh),all=hard&&realtimeOk&&displayFresh&&syncOk&&heartbeatOk;lastPreflight={all,hard,issues,latency:lat,backend:backend.data||null,displayFresh,syncOk,heartbeatOk,buildOk:versionsOk,realtimeOk,fallback:!realtimeOk&&hard,checked_at:Date.now(),automatic:quick};renderEventHealth();
    if(summary){summary.className=`preflight-summary ${all?'ready':hard?'warn':'fail'}`;summary.textContent=all?`PRONTO PARA EVENTO • ${BUILD_ID} • estado exato confirmado`:hard?`PRONTO COM ALERTA • ${issues.join(', ')||'operação em fallback'}`:`NÃO PRONTO • corrija ${issues.join(', ')||'itens críticos'}`;}
...
async function emergencyStopAction(){if(!room)return;const freezing=phase()==='question_open';if(freezing&&!confirm('Congelar a pergunta agora? O tempo restante será preservado e nenhuma nova resposta será aceita até retomar.'))return;await pauseQuiz();}
...
function journeyGo(step){if(step==='room'){switchAdminTab('config');setConfigPane('room',{scroll:true});}else if(step==='questions'){switchAdminTab('config');setConfigPane('questions',{scroll:true});}else if(step==='rules'){switchAdminTab('config');setConfigPane('rules',{scroll:true});}else if(step==='rounds'){switchAdminTab('config');setConfigPane('rounds',{scroll:true});}else if(step==='review'){switchAdminTab('config');setConfigPane('review',{scroll:true});renderStandardReview();}else if(step==='lobby'){openPresentationContext(presentationContextForRoom(),{scroll:false});setTimeout(()=>$('#adminLobbyCode')?.scrollIntoView({behavior:'smooth',block:'center'}),50);}}
function setupChecks(){
  const duplicateIds=queueRows.map(x=>x.question_id).filter((v,i,a)=>a.indexOf(v)!==i),invalid=queueRows.filter(q=>{const gt=String(q.game_type||q.game_type_snapshot||'standard');return !q.prompt||!q.question_type||(gt==='standard'&&q.question_type==='choice'&&![4,5].includes(Array.isArray(q.options)?q.options.length:Number(q.options_snapshot?.length||0)));}),checks=[{ok:!!room,label:'Sala criada',detail:room?`PIN ${room.code}`:'Crie a sala'},{ok:queueRows.length>0,label:'Perguntas selecionadas',detail:`${queueRows.length} na playlist`},{ok:duplicateIds.length===0,label:'Sem duplicações',detail:duplicateIds.length?`${duplicateIds.length} repetida(s)`:'Fila limpa'},{ok:invalid.length===0,label:'Perguntas válidas',detail:invalid.length?`${invalid.length} revisar`:'Gabaritos/alternativas OK'},{ok:!!room&&Number(roomSettings().default_time||30)>=5,label:'Regras configuradas',detail:`${roomSettings().default_time||30}s padrão • ${roomSettings().default_points||100} pts`}];
  if(presentationContextForRoom()==='pro'){const issues=validateGameQuestions(queueRows);checks.push({ok:issues.length===0,label:'Mecânicas GameShow completas',detail:issues.length?`${issues.length} pergunta(s): ${issues.slice(0,2).map(x=>x.issue).join(' • ')}`:'Todas as mecânicas possuem os dados necessários'});if(String(roomSettings().game_mode||'')==='teams')checks.push({ok:backendSchema()>=TEAM_FEATURE_SCHEMA_REQUIRED,label:'Backend para equipes',detail:backendSchema()>=TEAM_FEATURE_SCHEMA_REQUIRED?'Schema 043 disponível':'Execute 00_ATUALIZAR_SUPABASE_PARA_SCHEMA_043.sql'});}
  return checks;
}
function centralStepGo(step){
  const ctx=presentationContextForRoom();if(ctx==='pro'){switchAdminTab('pro');const map={room:'room',questions:'questions',teams:'teams',rules:'rules',rounds:'rounds',review:'review',lobby:'presentation'};window.quiz2ProBridge?.setPane?.(map[step]||'room',{remember:true,scroll:true});if(step==='lobby')window.quiz2AdminBridge?.mountPresentation?.('pro');return;}journeyGo(step);
}
function renderCentralJourney(){
  const nav=$('#setupJourney');if(!nav)return;const pro=presentationContextForRoom()==='pro',teams=pro&&String(roomSettings().game_mode||'')==='teams';
  const steps=pro?(teams?[['room','Criar','Sala + dinâmica'],['questions','Perguntas','Selecionar'],['teams','Equipes','Formar times'],['rules','Regras','Ritmo e pontos'],['rounds','Ordem','Sequência'],['review','Revisar','Validar tudo'],['lobby','Apresentar','Lobby e jogo']]:[['room','Criar','Sala + dinâmica'],['questions','Perguntas','Selecionar'],['rules','Regras','Ritmo e pontos'],['rounds','Ordem','Sequência'],['review','Revisar','Validar tudo'],['lobby','Apresentar','Lobby e jogo']]):[['room','Sala','Nome e visual'],['questions','Perguntas','Selecionar'],['rules','Regras','Ritmo e pontos'],['rounds','Ordem','Sequência'],['review','Revisar','Validar tudo'],['lobby','Apresentar','Lobby e jogo']];
  nav.classList.toggle('is-pro-flow',pro);nav.innerHTML=steps.map((x,i)=>`${i?'<i></i>':''}<button data-context-step="${x[0]}" type="button"><b>${i+1}</b><span>${x[1]}</span><small>${x[2]}</small></button>`).join('');
}
function renderSetupReview(){
  const list=$('#setupReviewList');if(!list)return;const checks=setupChecks(),done=checks.filter(x=>x.ok).length;renderCentralJourney();list.innerHTML=checks.map(x=>`<div class="setup-review-row ${x.ok?'ok':'warn'}"><span>${x.ok?'✓':'!'}</span><div><strong>${esc(x.label)}</strong><small>${esc(x.detail)}</small></div></div>`).join('');$('#setupReadinessBadge').textContent=`${done}/${checks.length}`;$('#setupReadinessBadge').className=`badge ${done===checks.length?'ready':'neutral'}`;$('#setupReviewSummary').textContent=done===checks.length?'Preparação consistente. Abra o lobby e execute o pré-teste antes de iniciar.':`Faltam ${checks.length-done} verificação(ões) para a preparação ficar completa.`;$('#openLobbyFromReviewBtn').disabled=done!==checks.length;$$('#setupJourney [data-context-step]').forEach(b=>{const step=b.dataset.contextStep,completed=step==='room'?!!room:step==='questions'?queueRows.length>0:step==='teams'?String(roomSettings().game_mode||'')==='teams':step==='rules'?!!room:step==='rounds'?queueRows.length>0:step==='review'?done===checks.length:step==='lobby'?!!room&&queueRows.length>0:false;b.classList.toggle('done',completed);});if($('#centralContinueHint'))$('#centralContinueHint').textContent=room?`${modeSummaryLabel()} • ${room.title} • ${statusLabel(phase())}`:'Nenhuma sala ativa';
}
function continueCurrentRoom(){if(!room)return openNewGameFlow();if(['question_open','preparing','result','paused','finished'].includes(phase()))return openPresentationContext(presentationContextForRoom(),{scroll:false});const ctx=presentationContextForRoom();if(ctx==='pro'){switchAdminTab('pro');window.quiz2ProBridge?.setPane?.(queueRows.length?'review':'room',{remember:true,scroll:true});}else{switchAdminTab('config');setConfigPane(queueRows.length?'review':'room',{scroll:true});}}
function reviewCurrentRoom(){if(!room)return openNewGameFlow();if(presentationContextForRoom()==='pro'){switchAdminTab('pro');window.quiz2ProBridge?.setPane?.('review',{remember:true,scroll:true});}else journeyGo('review');}
function renderBuildSummary(){const box=$('#eventBuildSummary');if(!box)return;box.classList.toggle('hidden',!room);if(!room)return;const active=queueRows,secs=active.reduce((n,q)=>n+Number(q.time_limit_seconds||q.time_limit_snapshot||30),0)+active.length*12,points=active.reduce((n,q)=>n+(q.score_enabled===false?0:Number(q.points||q.points_snapshot||0)),0),diffs={facil:0,medio:0,dificil:0,final:0};active.forEach(q=>diffs[q.difficulty||q.difficulty_snapshot||'medio']=(diffs[q.difficulty||q.difficulty_snapshot||'medio']||0)+1);$('#buildSummaryTitle').textContent=room.title||'Quiz';if($('#buildSummaryMode'))$('#buildSummaryMode').textContent=modeSummaryLabel();$('#buildSummaryQuestions').textContent=active.length;$('#buildSummaryDuration').textContent=`~${Math.max(1,Math.round(secs/60))} min`;$('#buildSummaryPoints').textContent=points.toLocaleString('pt-BR');$('#buildSummaryDifficulty').textContent=`${diffs.facil}F • ${diffs.medio}M • ${diffs.dificil+diffs.final}D`;if($('#playlistSummary'))$('#playlistSummary').innerHTML=`<span>${active.length} perguntas</span><span>~${Math.max(1,Math.round(secs/60))} min</span><span>${points.toLocaleString('pt-BR')} pts</span>`;renderSetupReview();renderStandardReview();}
...
function openNewGameFlow(){openNewGameChooser();}
...
$('#goQuestionsBtn').addEventListener('click',()=>{switchAdminTab('config');setConfigPane('questions',{scroll:true});});$('#configOpenPresentationBtn')?.addEventListener('click',()=>{switchAdminTab('config');setConfigPane('review',{scroll:true});});$('#standardReviewBackBtn')?.addEventListener('click',()=>setConfigPane('rounds',{scroll:true}));$('#standardTestGameBtn')?.addEventListener('click',launchGuidedTest);$('#standardSaveTemplateBtn')?.addEventListener('click',saveTemplate);$('#standardOpenPresentationBtn')?.addEventListener('click',()=>openPresentationContext('config'));$('#applyRecommendedRulesBtn')?.addEventListener('click',applyRecommendedRules);$('#startQuizBtn').addEventListener('click',startQuiz);$('#prepareRoundBtn').addEventListener('click',prepareRound);$('#openRoundBtn').addEventListener('click',openPreparedRound);$('#closeRoundBtn').addEventListener('click',closeRound);$('#pauseQuizBtn').addEventListener('click',pauseQuiz);$('#extend5Btn').addEventListener('click',()=>extendRound(5));$('#extend10Btn').addEventListener('click',()=>extendRound(10));$('#annulRoundBtn').addEventListener('click',annulRound);$('#regradeRoundBtn').addEventListener('click',regradeRound);$$('[data-reveal]').forEach(b=>b.addEventListener('click',()=>reveal(b.dataset.reveal)));$$('[data-final-show]').forEach(b=>b.addEventListener('click',()=>setFinalShowStage(b.dataset.finalShow)));$('#restartQuizBtn').addEventListener('click',restartQuiz);$('#displayBtn').addEventListener('click',()=>openDisplay('reuse'));$('#fullscreenDisplayBtn').addEventListener('click',()=>openDisplay('window'));$('#copyJoinLinkBtn').addEventListener('click',copyJoinLink);$('#forceSyncBtn').addEventListener('click',()=>recoverEventSync({silent:false}));$('#connectionTestBtn').addEventListener('click',runConnectionTest);$('#refreshParticipantsBtn').addEventListener('click',loadParticipants);$('#loadHistoryBtn').addEventListener('click',loadHistory);$('#showEvolutionBtn').addEventListener('click',showEvolution);
...
$('#simpleModeToggle')?.addEventListener('click',()=>applySimpleMode(!uiSimpleMode));$('#proSimpleModeBtn')?.addEventListener('click',()=>applySimpleMode(!uiSimpleMode));$('#standardShowAdvancedBtn')?.addEventListener('click',()=>{applySimpleMode(false);setTimeout(()=>document.querySelector('[data-standard-advanced="1"]')?.scrollIntoView({behavior:'smooth',block:'start'}),80);});$('#newGameCloseBtn')?.addEventListener('click',closeNewGameChooser);$('#newGameModal')?.addEventListener('click',e=>{if(e.target===$('#newGameModal'))closeNewGameChooser();});$$('[data-new-game]').forEach(b=>b.addEventListener('click',()=>chooseNewGame(b.dataset.newGame)));$('#centralNewGameBtn')?.addEventListener('click',openNewGameFlow);$('#centralContinueBtn')?.addEventListener('click',continueCurrentRoom);$('#centralBankBtn')?.addEventListener('click',()=>{switchAdminTab('questions');setQuestionPane('edit',{scroll:true});});$('#refreshRecentRoomsBtn')?.addEventListener('click',loadRecentRooms);$('#setupJourney')?.addEventListener('click',e=>{const b=e.target.closest('[data-context-step]');if(b)centralStepGo(b.dataset.contextStep);});$('#reviewBackRulesBtn')?.addEventListener('click',()=>journeyGo('rules'));$('#openLobbyFromReviewBtn')?.addEventListener('click',()=>journeyGo('lobby'));$('#buildSummaryReviewBtn')?.addEventListener('click',reviewCurrentRoom);$('#roomNextQuestionsBtn')?.addEventListener('click',()=>journeyGo('questions'));$('#rulesBackQuestionsBtn')?.addEventListener('click',()=>journeyGo('questions'));$('#rulesNextReviewBtn')?.addEventListener('click',async()=>{if(room)await saveRoomSettings(false);journeyGo('rounds');});$('#backToPreparationBtn')?.addEventListener('click',()=>{if(phase()!=='lobby'&&!confirm('A partida já começou. Voltar aos rounds apenas para consulta?'))return;if(presentationContext==='pro'){switchAdminTab('pro');window.quiz2ProBridge?.setPane?.('rounds',{remember:true,scroll:true});}else{switchAdminTab('config');setConfigPane('rounds',{scroll:true});}});$$('[data-rules-mode]').forEach(b=>b.addEventListener('click',()=>setRulesMode(b.dataset.rulesMode)));$$('[data-special-preset]').forEach(b=>b.addEventListener('click',()=>applySpecialPreset(b.dataset.specialPreset)));$('#questionBankCreateBtn')?.addEventListener('click',()=>{switchAdminTab('questions');setQuestionPane('edit',{scroll:true});});$('#questionBankImportBtn')?.addEventListener('click',()=>{switchAdminTab('questions');setQuestionPane('import',{scroll:true});});$('#questionBankBackBtn')?.addEventListener('click',()=>{if(questionBankContext==='pro'){switchAdminTab('pro');window.quiz2ProBridge?.setPane?.('room',{remember:true,scroll:true});}else{switchAdminTab('config');setConfigPane('room',{scroll:true});}});$('#questionBankNextBtn')?.addEventListener('click',()=>{if(questionBankContext==='pro'){switchAdminTab('pro');if(window.quiz2ProBridge?.nextFromQuestions)window.quiz2ProBridge.nextFromQuestions();else window.quiz2ProBridge?.setPane?.('rules',{remember:true,scroll:true});}else{switchAdminTab('config');setConfigPane('rules',{scroll:true});}});$('#questionOptionCountFilter')?.addEventListener('change',renderQuestionBank);$('#addRandom10Btn')?.addEventListener('click',()=>addRandomQuestions(10));$('#addCategoryBtn')?.addEventListener('click',addCategoryQuestions);$('#saveCollectionBtn')?.addEventListener('click',saveQuestionCollection);$('#loadCollectionBtn')?.addEventListener('click',loadCollectionToRoom);$('#deleteCollectionBtn')?.addEventListener('click',deleteQuestionCollection);$('#duplicateFinishedGameBtn')?.addEventListener('click',duplicateRoom);$('#differentQuestionsGameBtn')?.addEventListener('click',newRoomAfterGame);$$('[data-lobby-layout]').forEach(b=>b.addEventListener('click',()=>setLobbyLayout(b.dataset.lobbyLayout)));$$('[data-avatar-scene-live]').forEach(b=>b.addEventListener('click',()=>setAvatarSceneQuick(b.dataset.avatarSceneLive)));$$('[data-participant-filter]').forEach(b=>b.addEventListener('click',()=>{participantFilter=b.dataset.participantFilter||'all';$$('[data-participant-filter]').forEach(x=>x.classList.toggle('active',x===b));renderParticipantList();}));
$('#adminHelpBtn')?.addEventListener('click',()=>openQuestionHelp('helpQuick'));$('#questionHelpBtn')?.addEventListener('click',()=>openQuestionHelp('helpQuick'));$('#questionHelpCloseBtn')?.addEventListener('click',closeQuestionHelp);$('#questionHelpSearch')?.addEventListener('input',filterQuestionHelp);$$('#questionHelpModal [data-help-target]').forEach(btn=>btn.addEventListener('click',()=>document.getElementById(btn.dataset.helpTarget)?.scrollIntoView({behavior:'smooth',block:'start'})));$('#questionHelpOpenEditorBtn')?.addEventListener('click',()=>goFromHelp('edit'));$('#questionHelpOpenImportBtn')?.addEventListener('click',()=>goFromHelp('import'));$('#questionHelpModal')?.addEventListener('click',e=>{if(e.target===$('#questionHelpModal'))closeQuestionHelp();});document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!$('#newGameModal')?.classList.contains('hidden')){e.preventDefault();closeNewGameChooser();return;}if(e.key==='Escape'&&!$('#questionHelpModal')?.classList.contains('hidden')){e.preventDefault();closeQuestionHelp();return;}trapQuestionHelpFocus(e);});
initSimpleMode();window.addEventListener('online',()=>{renderEventHealth();room&&subscribe();});window.addEventListener('offline',()=>{conn('offline');setPoll(POLL_FALLBACK);renderEventHealth();});document.addEventListener('visibilitychange',()=>{if(document.hidden)return;if(remoteModeActive){syncRemoteNow();return;}if(room){Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants()]);eventWatchdogTick();}});window.addEventListener('load',authCheck);
