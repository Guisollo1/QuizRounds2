...
export const BUILD_ID='3.68-r88';
...
export function storageGet(key,fallback=''){try{const value=localStorage.getItem(key);return value===null?fallback:value;}catch{return fallback;}}
export function storageSet(key,value){try{localStorage.setItem(key,String(value));return true;}catch{return false;}}
export function storageRemove(key){try{localStorage.removeItem(key);return true;}catch{return false;}}
export function jitterDelay(ms,spread=.2,min=700){const base=Math.max(0,Number(ms)||0),range=Math.max(0,Number(spread)||0);return Math.max(min,Math.round(base*(1+((Math.random()*2)-1)*range)));}
export function isOlderRoomState(incoming,current){
  if(!incoming||!current)return false;
  const incomingGeneration=Number(incoming.generation||0),currentGeneration=Number(current.generation||0);
  if(incomingGeneration!==currentGeneration)return incomingGeneration<currentGeneration;
  return Number(incoming.state_version||0)<Number(current.state_version||0);
}
...
let feedbackAudioEnabled=true,feedbackAudioCtx=null,feedbackAudioMaster=null,feedbackAudioUnlocked=false,feedbackAudioVolume=.82,feedbackAmbientTimer=null,feedbackAmbientStep=0,feedbackAmbientProfile='show',feedbackAmbientVolume=.55;
...
function feedbackAudioContext(){
  ...
  try{
    ...
    if(!feedbackAudioCtx){feedbackAudioCtx=new C();feedbackAudioMaster=feedbackAudioCtx.createGain();feedbackAudioMaster.gain.value=feedbackAudioVolume;feedbackAudioMaster.connect(feedbackAudioCtx.destination);}
    return feedbackAudioCtx;
  }catch{return null;}
}
function publishDisplayAudioState(){if(!isDisplayAudioScope())return;document.body.dataset.displaySoundEnabled=feedbackAudioEnabled?'1':'0';document.body.dataset.displaySoundVolume=String(feedbackAudioVolume);try{window.dispatchEvent(new CustomEvent('quizrounds:display-audio-state',{detail:{enabled:feedbackAudioEnabled,volume:feedbackAudioVolume}}));}catch{}}
function applyFeedbackMasterVolume(){if(feedbackAudioMaster&&feedbackAudioCtx){const now=feedbackAudioCtx.currentTime;feedbackAudioMaster.gain.cancelScheduledValues(now);feedbackAudioMaster.gain.setTargetAtTime(feedbackAudioVolume,now,.025);}publishDisplayAudioState();}
export function setDisplayAudioVolume(value=.82){if(!isDisplayAudioScope())return false;const n=Number(value);feedbackAudioVolume=Math.max(0,Math.min(1.25,Number.isFinite(n)?n:.82));applyFeedbackMasterVolume();return feedbackAudioVolume;}
export function setFeedbackAudioEnabled(value){
  ...
  if(!feedbackAudioEnabled){stopDisplayAmbience();if(feedbackAudioCtx?.state==='running')feedbackAudioCtx.suspend?.().catch(()=>{});} 
  publishDisplayAudioState();return feedbackAudioEnabled;
}
export async function unlockFeedbackAudio(){
  ...
  applyFeedbackMasterVolume();
  publishDisplayAudioState();return ctx.state==='running';
}
...
export function playFeedbackCue(kind='tap',opts={}){
  ...
  const gain=Math.max(.25,Math.min(1.5,Number(opts.volume)||1)),cueBoost=1.65,T=(f,d,a,delay=0,type='sine',to=null)=>cueTone({f,d,a:a*gain*cueBoost,delay,type,to});
  switch(kind){
    case'audioTest':T(523,.10,.034);T(784,.12,.042,.12,'triangle');T(1047,.18,.050,.27,'triangle');break;
...
function ambientProfileNotes(profile){return({clean:[220,330,440],show:[196,294,392,523.25],corporate:[220,277,330,440],arcade:[262,330,392,523.25]})[profile]||[196,294,392,523.25];}
function ambientPulse(){
  const ctx=feedbackAudioContext();if(!ctx||!feedbackAudioMaster||!feedbackAudioEnabled||!feedbackAudioUnlocked||ctx.state!=='running')return;
  const notes=ambientProfileNotes(feedbackAmbientProfile),base=notes[feedbackAmbientStep++%notes.length],at=ctx.currentTime;
  try{
    [base,base*1.5].forEach((f,i)=>{const o=ctx.createOscillator(),g=ctx.createGain();o.type=i?'triangle':'sine';o.frequency.value=f;const peak=(i ? .0045 : .008)*feedbackAmbientVolume;g.gain.setValueAtTime(.0001,at+i*.08);g.gain.exponentialRampToValueAtTime(Math.max(.0002,peak),at+.09+i*.08);g.gain.exponentialRampToValueAtTime(.0001,at+1.15+i*.08);o.connect(g).connect(feedbackAudioMaster);o.start(at+i*.08);o.stop(at+1.2+i*.08);});
  }catch{}
}
export function startDisplayAmbience(opts={}){
  if(!isDisplayAudioScope()||!feedbackAudioEnabled||!feedbackAudioUnlocked)return false;
  feedbackAmbientProfile=['clean','show','corporate','arcade'].includes(opts.profile)?opts.profile:'show';
  const v=Number(opts.volume);feedbackAmbientVolume=Math.max(0,Math.min(1,Number.isFinite(v)?v:.55));
  if(feedbackAmbientTimer)return true;
  ambientPulse();feedbackAmbientTimer=setInterval(ambientPulse,Math.max(1400,Number(opts.interval)||2200));return true;
}
export function stopDisplayAmbience(){if(feedbackAmbientTimer){clearInterval(feedbackAmbientTimer);feedbackAmbientTimer=null;}feedbackAmbientStep=0;return true;}
