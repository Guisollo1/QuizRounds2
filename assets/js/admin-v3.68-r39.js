import{db,$,esc,renderRank,setConnection,serverRemaining,sleep,APP_VERSION,BUILD_ID,BACKEND_SCHEMA_REQUIRED,applyDocumentTheme,normalizeLogoUrl}from'./common-v3.68-r39.js';
import{avatarMarkup}from'./avatars-v3.68-r39.js';
...
function loadAdminMotionAfterAuth(){
  ...
  if(!document.getElementById('adminMotionCss')){const link=document.createElement('link');link.id='adminMotionCss';link.rel='stylesheet';link.href='assets/css/motion-v3.68-r39.css?v=3.68-r39';document.head.appendChild(link);}
  import('./motion-v3.68-r39.js?v=3.68-r39').catch(()=>{});
}
...
function renderDiagnostics(){const el=$('#diagnosticLog');if(!el)return;el.innerHTML=diagnosticEvents.slice(0,80).map(x=>`<div class="diagnostic-row ${esc(x.type)}"><time>${new Date(x.at).toLocaleTimeString('pt-BR')}</time><strong>${esc(x.message)}</strong><span>${esc(Object.keys(x.details||{}).length?JSON.stringify(x.details):'')}</span></div>`).join('')||'<div class="empty">Sem eventos técnicos nesta sessão.</div>';}
...
function exportDiagnostics(){const lines=['QuizRounds '+BUILD_ID+' — diagnóstico do evento',`Sala: ${room?.code||'sem sala'}`,`Gerado: ${new Date().toISOString()}`,'',...diagnosticEvents.slice().reverse().map(x=>`${x.at} | ${x.type.toUpperCase()} | ${x.message}${Object.keys(x.details||{}).length?' | '+JSON.stringify(x.details):''}`)];downloadText(`QuizRounds_diagnostico_${room?.code||'sem_sala'}_${new Date().toISOString().replace(/[:.]/g,'-')}.txt`,lines.join('\n'));}
...
function updateSettingsSaveIndicator(state='idle'){
  const btn=$('#saveRoomSettingsBtn');if(!btn)return;
  if(state==='saving'){btn.textContent='Salvando…';btn.disabled=true;return;}
  btn.disabled=false;
  if(state==='dirty'){btn.textContent='Salvar configurações • alterações';return;}
  if(state==='error'){btn.textContent='Salvar configurações • tentar novamente';return;}
...
function renderEventHealth(){
  ...
  if($('#displayBtn'))$('#displayBtn').textContent=displayOnline?'Abrir telão / projetor':'Reabrir telão / projetor';
}
