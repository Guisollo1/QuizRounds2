...
let presentationContext='config',questionBankContext='config',uiSimpleMode=true,reuseQuestionsForNextRoom=false,exampleGamePromise=null;
...
function loadAdminMotionAfterAuth(){
  ...
  if(!document.getElementById('adminMotionCss')){const link=document.createElement('link');link.id='adminMotionCss';link.rel='stylesheet';link.href='assets/css/motion-v3.68-r90.css?v=3.68-r90';document.head.appendChild(link);}
  import('./motion-v3.68-r90.js?v=3.68-r90').catch(()=>{});
}
...
async function setFinalShowStage(stage){if(!room||phase()!=='finished')return;stage=String(stage||'');if(!['ranking','stats','highlights','champion'].includes(stage))return;const label=finalShowLabel(stage);await withAction(`Mostrando ${label.toLowerCase()}…`,async()=>{const{error}=await db.rpc('admin_set_final_show_stage',{p_room_id:room.id,p_stage:stage});if(error)throw error;let synced=await refreshState(true);if(!synced||finalShowStage()!==stage){await new Promise(r=>setTimeout(r,120));synced=await refreshState(true);}if(!synced||finalShowStage()!==stage)throw new Error('O servidor confirmou a ação, mas o ADM ainda não recebeu a etapa final atualizada. Tente novamente.');await Promise.all([loadEventInsights(),broadcastSyncNudge('final-show-stage',{event:'final_show_changed',stage,final_show_stage:stage})]);remoteVibrate(stage==='champion'?[90,35,130]:[35,25,55]);});}
...
async function syncStateToVersion(expected=0,{force=true}={}){
  ...
  realtimeSyncPromise=(async()=>{
    ...
    try{
      while(pass<4){
        pass++;
        await refreshState(pass===1?force:false);
        const wanted=Number(realtimeTargetVersion||0),local=realtimeStateVersion();
        if(!wanted||local>=wanted){realtimeTargetVersion=0;return true;}
        if(pass<4)await sleep(90+pass*90);
      }
      ...
      if(realtimeTargetVersion>realtimeStateVersion()&&realtimeRetryBudget>0&&!realtimeRetryTimer){
        realtimeRetryBudget--;
        realtimeRetryTimer=setTimeout(()=>{realtimeRetryTimer=null;if(realtimeTargetVersion>realtimeStateVersion())syncStateToVersion(realtimeTargetVersion,{force:false});},900);
      }
    }
  })();
...
async function broadcastSyncNudge(reason='manual',extra={}){if(!room||!channel||!lastConnectionKey.startsWith('connected'))return false;const payload={state_version:Number(roomState?.room?.state_version||0),generation:Number(roomState?.room?.generation||1),source:'admin-sync-recovery',reason,at:Date.now(),...(extra&&typeof extra==='object'?extra:{})};try{await channel.send({type:'broadcast',event:'state_changed',payload});diag('sync','Solicitação de convergência enviada',{reason,state_version:payload.state_version,generation:payload.generation,stage:payload.stage||null});return true;}catch(e){diag('warn','Falha ao solicitar convergência',{reason,error:e?.message||String(e)});return false;}}
async function recoverEventSync({silent=false,timeoutMs=2200}={}){
  if(!room||!channel||!lastConnectionKey.startsWith('connected')){const result={ok:false,reason:'Realtime não está conectado',snapshot:syncSnapshot(),test:null};if(!silent)actionToast('Realtime ainda não está conectado.','error');return result;}
  await refreshState(true);
  await broadcastSyncNudge('manual');
  await sleep(Math.min(900,Math.max(300,Number(timeoutMs)||900)));
  await refreshState(false);
  const final=syncSnapshot();
  const result={ok:!versionMismatch&&lastConnectionKey.startsWith('connected'),reason:final.lagging?'Presence ainda atualizando; o estado do jogo já foi solicitado.':'',snapshot:final,test:null};
  if(!silent)actionToast(result.ok?(final.lagging?'Sincronização solicitada; Presence será atualizada automaticamente.':'Aparelhos sincronizados.'):'Não foi possível solicitar sincronização.',result.ok?'ok':'error');
  renderEventHealth();
  return result;
}
...
async function eventWatchdogTick(){if(!room){lastWatchdogIssue='';return;}const snap=watchdogSnapshot(),issues=[];if(!snap.online)issues.push('sem internet');else if(!snap.realtimeOk)issues.push('Realtime reconectando');if(!snap.buildOk)issues.push('builds diferentes');if(snap.lag.displayStale)issues.push('telão sem heartbeat');if(snap.lag.playerStale)issues.push(`${snap.lag.playerStale} jogador${snap.lag.playerStale===1?'':'es'} sem heartbeat`);if(snap.active&&!displayOnline)issues.push('telão desconectado durante a partida');const key=issues.join('|');if(key!==lastWatchdogIssue){if(key)diag('warn','Watchdog detectou condição degradada',{issues,state_version:snap.lag.currentVersion});else if(lastWatchdogIssue)diag('ok','Watchdog: operação voltou ao normal',{state_version:snap.lag.currentVersion});lastWatchdogIssue=key;}renderEventHealth();}
async function subscribe(){
  ...
  progress.subscribe(status=>{
    ...
    if(['CHANNEL_ERROR','TIMED_OUT','CLOSED'].includes(status))diag('warn','Canal auxiliar de progresso indisponível; canal principal mantido',{status});
  });
...
function renderContextAction(){
  ...
  else if(p==='lobby'){label='INICIAR QUIZ';text='Valida a sala sem reiniciar conexões e inicia a contagem do primeiro round.';enabled=queueRows.length>0;}
...
const EXAMPLE_GAME_QUESTIONS=[
  {prompt:'Qual planeta é conhecido como Planeta Vermelho?',type:'choice',options:[{key:'A',text:'Marte'},{key:'B',text:'Vênus'},{key:'C',text:'Júpiter'},{key:'D',text:'Mercúrio'}],correctChoice:'A',correctNumber:null,points:100,time:20,category:'Exemplo',difficulty:'facil',notes:'Pergunta criada automaticamente pela Partida de exemplo.'},
  {prompt:'Quanto é 12 × 10?',type:'numeric',options:null,correctChoice:null,correctNumber:120,points:100,time:20,category:'Exemplo',difficulty:'facil',notes:'Pergunta numérica de exemplo. Gabarito: 120.'}
];
async function ensureExampleQuestionIds(){
  await loadQuestions();
  const ids=[];
  for(const spec of EXAMPLE_GAME_QUESTIONS){
    let existing=questionBank.find(q=>String(q.prompt||'').trim()===spec.prompt&&String(q.question_type||'')===spec.type&&!q.archived);
    if(existing){ids.push(existing.id);continue;}
    const created=await db.rpc('admin_create_question_v2',{p_prompt:spec.prompt,p_type:spec.type,p_options:spec.options,p_correct_choice:spec.correctChoice,p_correct_number:spec.correctNumber,p_points:spec.points,p_time_limit:spec.time});
    if(created.error)throw created.error;
    const id=created.data;
    const meta=await db.rpc('admin_update_question_metadata',{p_question_id:id,p_category:spec.category,p_difficulty:spec.difficulty,p_score_enabled:true,p_speed_bonus_pct:0,p_is_tiebreaker:false,p_presenter_notes:spec.notes});
...
async function createExampleGame(){
  if(exampleGamePromise){actionToast('A Partida de exemplo já está sendo criada.','neutral');return exampleGamePromise;}
  const task=(async()=>{
    const previousRoomId=String(room?.id||'');
    const btn=$('#createExampleGameBtn');if(btn)btn.disabled=true;
    const title=$('#roomTitle'),rounds=$('#plannedRounds');
    if(title)title.value='Partida de exemplo';
    if(rounds)rounds.value='2';
    reuseQuestionsForNextRoom=false;
    if($('#newGameReuseQuestions'))$('#newGameReuseQuestions').checked=false;
    await createRoom();
    if(!room?.id||(previousRoomId&&String(room.id)===previousRoomId))return false;
    const ids=await ensureExampleQuestionIds();
    for(const id of ids){const queued=await db.rpc('admin_queue_question',{p_room_id:room.id,p_question_id:id});if(queued.error)throw queued.error;}
    const planned=await db.rpc('admin_update_planned_rounds',{p_room_id:room.id,p_planned_rounds:2});if(planned.error)throw planned.error;
    await Promise.all([refreshState(true),loadQueue(),loadAudit(),loadParticipants()]);
    if($('#plannedRounds'))$('#plannedRounds').value='2';
    journeyGo('lobby');
    actionToast('Partida de exemplo pronta: 2 perguntas e lobby aberto ✓');
    return true;
  })();
  exampleGamePromise=task;
  try{return await task;}catch(e){diag('error','Falha ao criar Partida de exemplo',{error:e?.message||String(e)});actionToast(e?.message||'Falha ao criar Partida de exemplo','error');alert(e?.message||String(e));return false;}finally{if(exampleGamePromise===task)exampleGamePromise=null;const btn=$('#createExampleGameBtn');if(btn)btn.disabled=false;}
}
