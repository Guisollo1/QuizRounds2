# QuizRounds2 v3.68-r91 — Base funcional r67

Esta release parte diretamente da r67 fornecida como referência funcional. Ela adiciona apenas a Partida de exemplo de 2 perguntas com abertura direta do lobby e o lobby do telão em coluna única (PIN, QR e números/status empilhados).

Consulte `COMPARATIVO_SEPARADO_r89_vs_r91_Base_r67.txt` para ver, separadamente, o que existia na r89 e não foi importado automaticamente.

# QuizRounds2 v3.68-r91 — CI Consistente + Publicação Verificada

## Estado desta release

A r91 consolida o runtime pré-publicação com um único sistema de áudio exclusivo do telão e mantém a verificação automática do conteúdo realmente servido pelo GitHub Pages. Não cria migration nova e mantém o backend base no schema 042, com schema 043 necessário somente para os três modos avançados de formação de equipes.

### Refatoração r91

- Runtime ativo unificado em **r91**: HTML, JS, CSS, cache-bust, `/admin`, workflow, validador e diagnóstico usam a mesma revisão.
